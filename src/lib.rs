    ...
    E1: CurveCycleEquipped,
{
    ...
    /// * `ck_hint1`: A `CommitmentKeyHint` for `G1`, which is a function that
    ///   provides a hint for the number of generators required in the
    ///   commitment scheme for the primary circuit.
    /// * `ck_hint2`: A `CommitmentKeyHint` for `G2`, similar to `ck_hint1`, but
    ...
    pub fn setup<C1: StepCircuit<E1::Scalar>, C2: StepCircuit<<Dual<E1> as Engine>::Scalar>>(
        ...
        let ro_consts_circuit_secondary: ROConstantsCircuit<E1> =
    ...
    E1: CurveCycleEquipped,
{
    ...
    pub fn verify(
        ...
        let is_inputs_not_match =
            self.z0_primary != z0_primary || self.z0_secondary != z0_secondary;
    ...
    S2: RelaxedR1CSSNARKTrait<Dual<E1>>,
{
    ...
    pub fn verify(
        ...
        let (res_primary, res_secondary) = rayon::join(
            || {
                self.r_W_snark_primary
                    .verify(&vk.vk_primary, &self.r_U_primary)
            },
            || {
                self.f_W_snark_secondary
