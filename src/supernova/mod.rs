...
use crate::{
    ...
    nifs::NIFS,
    r1cs::{
        self, commitment_key_size, CommitmentKeyHint, R1CSInstance, R1CSResult, R1CSShape,
        R1CSWitness, RelaxedR1CSInstance, RelaxedR1CSWitness,
    },
    scalar_as_base,
    traits::{
        commitment::{CommitmentEngineTrait, CommitmentTrait},
        AbsorbInROTrait, CurveCycleEquipped, Dual, Engine, ROConstants, ROConstantsCircuit,
        ROTrait,
    },
    ...
    E1: CurveCycleEquipped,
{
    ...
    /// * `ck_hint1`: A `CommitmentKeyHint` for `E1`, which is a function that
    ///   provides a hint for the number of generators required in the
    ///   commitment scheme for the primary circuit.
    /// * `ck_hint2`: A `CommitmentKeyHint` for `E2`, similar to `ck_hint1`, but
    ...
    pub fn digest(&self) -> E1::Scalar {
        self.digest
            .get_or_try_init(|| {
                let dc: DigestComputer<'_, <E1 as Engine>::Scalar, Self> =
    ...
    E1: CurveCycleEquipped,
{
    ...
    >(
        ...
        let binding = Commitment::<E1>::decompress(&nifs_primary.comm_T)
            .map_err(SuperNovaError::NovaError)?;
        let inputs_secondary: SuperNovaAugmentedCircuitInputs<'_, E1> =
            SuperNovaAugmentedCircuitInputs::new(
                self.pp_digest,
        ...
        let circuit_secondary: SuperNovaAugmentedCircuit<'_, E1, C2> =
            SuperNovaAugmentedCircuit::new(
                &pp.augmented_circuit_params_secondary,
                Some(inputs_secondary),
                c_secondary,
    ...
    pub fn verify(
        ...
        let hash_primary = {
            ...
            let mut hasher =
                <Dual<E1> as Engine>::RO::new(pp.ro_consts_secondary.clone(), num_absorbs);
