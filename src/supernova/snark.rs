    ...
    S2: RelaxedR1CSSNARKTrait<Dual<E1>>,
{
    ...
    pub fn prove(
        ...
        let r_U_primary = recursive_snark
            ...
            .enumerate()
            .map(|(idx, r_U)| {
                r_U.clone().unwrap_or_else(|| {
                    RelaxedR1CSInstance::default(&*pp.ck_primary, &pp[idx].r1cs_shape)
                })
            })
            .collect::<Vec<_>>();
...
mod test {
    ...
    impl<E: Engine> StepCircuit<E::Scalar> for SquareCircuit<E> {
        ...
        > {
            ...
            let next_pc =
                AllocatedNum::alloc(cs.namespace(|| "next_pc"), || Ok(E::Scalar::from(1u64)))?;
    ...
    impl<E: Engine> StepCircuit<E::Scalar> for CubeCircuit<E> {
        ...
        > {
            ...
            let next_pc =
                AllocatedNum::alloc(cs.namespace(|| "next_pc"), || Ok(E::Scalar::from(0u64)))?;
    ...
    fn test_compression_with_circuit_size_difference() {
        ...
        test_compression_with::<Bn256EngineIPA, S1PP<_>, S2<_>, _, _>(
            NUM_STEPS,
            BigTestCircuit::new,
        );
