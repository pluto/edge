...
impl<E: Engine, EE: EvaluationEngineTrait<E>> RelaxedR1CSSNARKTrait<E> for RelaxedR1CSSNARK<E, EE> {
    ...
    fn prove(
        ...
        // outer sum-check
        let tau: EqPolynomial<_> =
            PowPolynomial::new(&transcript.squeeze(b"t")?, num_rounds_x).into();
        ...
        let comb_func_outer = |poly_A_comp: &E::Scalar,
...
> {
    ...
    // P = ∑ᵢ γⁱ⋅Pᵢ
    let w_joint =
        PolyEvalWitness::batch_diff_size(&w_vec.iter().by_ref().collect::<Vec<_>>(), gamma);
