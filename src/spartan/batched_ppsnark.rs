    ...
    for BatchedRelaxedR1CSSNARK<E, EE>
{
    ...
    fn prove(
        ...
        let W =
            zip_with!(par_iter, (W, S), |w, s| w.pad(s)).collect::<Vec<RelaxedR1CSWitness<E>>>();
        ...
        let (comms_W_E, us): (Vec<_>, Vec<_>) =
            U.iter().map(|U| ([U.comm_W, U.comm_E], U.u)).unzip();
        ...
        let outer_sc_inst = zip_with!(
            ...
            |poly_ABC, poly_E, poly_Mz, poly_tau, eval_Mz, u| {
                ...
                let poly_uCz_E =
                    zip_with!(par_iter, (poly_Cz, poly_E), |cz, e| *u * cz + e).collect();
        ...
        let (mem_sc_inst, comms_mem_oracles, polys_mem_oracles) = {
            ...
            let (comms_mem_oracles, polys_mem_oracles, mem_aux) = pk
                ...
                .try_fold(
                    ...
                    |(mut comms, mut polys, mut aux),
                     (((s_repr, poly_tau), poly_Z), [L_row, L_col])| {
        ...
        let (sc, rand_sc, claims_outer, claims_inner, claims_mem, claims_witness) =
            Self::prove_helper(
                num_rounds_sc,
                mem_sc_inst,
                outer_sc_inst,
            ...
            |num_vars,
             ...
             eval_mem_preprocessed| {
                ...
                let (eq_tau, eq_masked_tau) = {
                    ...
                    let eq_masked_tau =
                        MaskedEqPolynomial::new(&eq_tau, num_vars_log).evaluate(rand_sc);
