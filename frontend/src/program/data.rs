...
// /// Fold input for any circuit containing signals name and vector of values. Inputs are
// distributed /// evenly across folds after the ROM is finalised by the prover.
...
//       let chunks: Vec<Vec<Value>> = value.chunks(chunk_size).map(|chunk|
// chunk.to_vec()).collect();       for i in 0..freq {
...
//   /// Converts proving instance parameters into an expanded form by distributing fold inputs
// across   /// their corresponding circuit instances in the ROM.
//   ///
...
//   /// # Details
//   ///
//   /// The expansion process handles fold inputs, which are inputs that need to be distributed
// across   /// multiple instances of the same circuit in the ROM. For each circuit label in the
// inputs:   /// 1. Finds all positions of that circuit in the ROM
...
//   /// Converts an offline setup parameters instance back into an online version by decompressing
// and   /// deserializing the public parameters and reconstructing the circuit shapes.
//   ///
//   /// This method performs the following steps:
...
//     let memory = Switchboard { circuits };
...
//     let result = setup_params.extend_public_inputs(&proof_params.rom,
// &instance_params.nivc_input);     assert!(result.is_ok());
...
//     );
//     assert_eq!(z0_primary[instance_params.nivc_input.len()], F::<G1>::ZERO); // Check ROM index
// is 0
...
//     let result = setup_params.extend_public_inputs(&proof_params.rom,
// &instance_params.nivc_input);     assert!(result.is_err());
//     assert!(matches!(
//         result.unwrap_err(),
