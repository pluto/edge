//! # Program Module
//!
//! The `program` module contains the core logic for setting up and running the
//! proof system. It provides functionality for initializing the setup,
...
///
/// This struct contains the circuits and ROM data required for the proof
/// system.
pub struct Memory {
...
///
/// This struct encapsulates the necessary components and metadata for a ROM
/// circuit, including the circuit itself, its index, size, and various inputs.
#[derive(Clone)]
...
// NOTE (Colin): This is added so we can cache only the active circuits we are
// using.
...
/// Executes the SuperNova program with the provided setup, proof, and instance
/// parameters.
///
/// This function initializes the public inputs, resizes the ROM, and
/// iteratively processes each step of the ROM to generate a recursive SNARK
/// proof. It handles the private inputs, witness generation, and circuit
/// execution for each opcode in the ROM.
///
/// # Arguments
///
...
/// # Returns
///
/// A `Result` containing the generated `RecursiveSNARK` on success, or a
/// `ProofError` on failure.
///
/// # Errors
///
/// This function can return a `ProofError` if there are issues with the NIVC
/// input, private inputs, or witness generation.
pub async fn run(
  ...
  // TODO (Colin): We are basically creating a `R1CS` for each circuit here, then
  // also creating `R1CSWithArity` for the circuits in the `PublicParams`.
  // Surely we don't need both?
    ...
    resized_rom.iter().enumerate().take_while(|(_, &op_code)| op_code != u64::MAX)
  {
    ...
    // trace!("private input: {:?}", memory.circuits[op_code as
    // usize].private_input);
