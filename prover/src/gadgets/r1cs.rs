...
impl<E: Engine, const N: usize> AllocatedR1CSInstance<E, N> {
  ...
  pub fn alloc<CS: ConstraintSystem<<E as Engine>::Base>>(
    ...
    let W =
      AllocatedPoint::alloc(cs.namespace(|| "allocate W"), u.map(|u| u.comm_W.to_coordinates()))?;
...
impl<E: Engine, const N: usize> AllocatedRelaxedR1CSInstance<E, N> {
  ...
  pub fn alloc<CS: ConstraintSystem<<E as Engine>::Base>>(
    ...
    // We do not need to check that W or E are well-formed (e.g., on the curve) as
    // we do a hash check in the Nova augmented circuit, which ensures that
    // the relaxed instance came from a prior iteration of Nova.
  ...
  pub fn absorb_in_ro<CS: ConstraintSystem<<E as Engine>::Base>>(
    ...
    self.X.iter().enumerate().try_for_each(|(idx, X)| {
      X.as_limbs().iter().enumerate().try_for_each(|(i, limb)| -> Result<(), SynthesisError> {
        ro.absorb(
          &limb
            .as_allocated_num(cs.namespace(|| format!("convert limb {i} of X_r[{idx}] to num")))?,
        );
        Ok(())
      })
    })?;
  ...
  pub fn fold_with_r1cs<CS: ConstraintSystem<<E as Engine>::Base>>(
    ...
    Ok(Self { W: W_fold, E: E_fold, u: u_fold, X: X_fold })
  }

  /// If the condition is true then returns this otherwise it returns the
  /// other
...
>(
  ...
  let c = AllocatedRelaxedR1CSInstance {
    W: conditionally_select_point(cs.namespace(|| "W = cond ? a.W : b.W"), &a.W, &b.W, condition)?,
    E: conditionally_select_point(cs.namespace(|| "E = cond ? a.E : b.E"), &a.E, &b.E, condition)?,
    u: conditionally_select(cs.namespace(|| "u = cond ? a.u : b.u"), &a.u, &b.u, condition)?,
    X: c_X,
  };
  Ok(c)
}
...
>(
  ...
  a.iter()
    ...
    .zip_eq(b.iter())
    .map(|((i, a), b)| {
      a.conditionally_select(cs.namespace(|| format!("cond ? a[{}]: b[{}]", i, i)), b, condition)
    })
