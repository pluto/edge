...
/// Gets as input the little indian representation of a number and spits out the
/// number
...
pub fn alloc_zero<F: PrimeField, CS: ConstraintSystem<F>>(mut cs: CS) -> AllocatedNum<F> {
  ...
  cs.enforce(|| "check zero is valid", |lc| lc, |lc| lc, |lc| lc + zero.get_variable());
  zero
}
...
pub fn alloc_num_equals<F: PrimeField, CS: ConstraintSystem<F>>(
  ...
  let t = AllocatedNum::alloc(cs.namespace(|| "t"), || {
    ...
    Ok(if a_val == b_val { F::ONE } else { (a_val - b_val).invert().unwrap() })
  })?;
...
pub fn conditionally_select_allocated_bit<F: PrimeField, CS: ConstraintSystem<F>>(
  ...
  let c = AllocatedBit::alloc(
    ...
    if condition.get_value().unwrap_or(false) { a.get_value() } else { b.get_value() },
  )?;
