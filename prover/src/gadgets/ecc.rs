...
impl<G: Group> AllocatedPoint<G> {
  ...
  pub fn alloc<CS: ConstraintSystem<G::Base>>(
    ...
    let x =
      AllocatedNum::alloc(cs.namespace(|| "x"), || Ok(coords.map_or(G::Base::ZERO, |c| c.0)))?;
    let y =
  ...
  where CS: ConstraintSystem<G::Base> {
  ...
  pub fn default<CS: ConstraintSystem<G::Base>>(mut cs: CS) -> Self {
    ...
    Self { x: zero.clone(), y: zero, is_infinity: one }
  }
  ...
  pub const fn get_coordinates(
    &self,
  ) -> (&AllocatedNum<G::Base>, &AllocatedNum<G::Base>, &AllocatedNum<G::Base>) {
  ...
  pub fn negate<CS: ConstraintSystem<G::Base>>(&self, mut cs: CS) -> Result<Self, SynthesisError> {
    ...
    Ok(Self { x: self.x.clone(), y, is_infinity: self.is_infinity.clone() })
  }
  ...
  pub fn add<CS: ConstraintSystem<G::Base>>(
    ...
    let equal_x = alloc_num_equals(cs.namespace(|| "check self.x == other.x"), &self.x, &other.x)?;

    let equal_y = alloc_num_equals(cs.namespace(|| "check self.y == other.y"), &self.y, &other.y)?;
  ...
  pub fn add_internal<CS: ConstraintSystem<G::Base>>(
    ...
    let x_diff =
      select_one_or_diff2(cs.namespace(|| "Compute x_diff"), &other.x, &self.x, &x_diff_is_actual)?;
    ...
    //************************************************************************/
    // We only return the computed x, y if neither of the points is infinity and
    // self.x != other.y if self.is_infinity return other.clone()
  ...
  pub fn double<CS: ConstraintSystem<G::Base>>(&self, mut cs: CS) -> Result<Self, SynthesisError> {
    ...
    // Now compute lambda as (G::Base::from(3) * self.x * self.x + G::GG::A()) *
    // tmp_inv
  ...
  pub fn scalar_mul<CS: ConstraintSystem<G::Base>>(
    ...
    // we convert AllocatedPoint into AllocatedPointNonInfinity; we deal with the
    // case where self.is_infinity = 1 below
    let mut p = AllocatedPointNonInfinity::from_allocated_point(self);

    // we assume the first bit to be 1, so we must initialize acc to self and double
    // it we remove this assumption below
    let mut acc = p;
    p = acc.double_incomplete(cs.namespace(|| "double"))?;

    // perform the double-and-add loop to compute the scalar mul using incomplete
    // addition law
    ...
    let res = {
      ...
      // we remove the initial slack if bits[0] is as not as assumed (i.e., it is not
      // 1)
    ...
    // we already set res.is_infinity to be self.is_infinity, so we do not need to
    // set it here
    ...
    let mut acc = Self { x, y, is_infinity: res.is_infinity };
  ...
  pub fn select_point_or_infinity<CS: ConstraintSystem<G::Base>>(
    ...
    let is_infinity =
      select_num_or_one(cs.namespace(|| "select is_infinity"), &a.is_infinity, condition)?;
...
impl<G: Group> AllocatedPointNonInfinity<G> {
  ...
  #[allow(unused)]
  pub const fn new(x: AllocatedNum<G::Base>, y: AllocatedNum<G::Base>) -> Self { Self { x, y } }
...
mod tests {
  ...
  impl<G: Group> Point<G> {
    pub fn new(x: G::Base, y: G::Base, is_infinity: bool) -> Self { Self { x, y, is_infinity } }
    ...
    pub fn random_vartime() -> Self {
      loop {
        ...
        if y.is_some().unwrap_u8() == 1 {
          return Self { x, y: y.unwrap(), is_infinity: false };
        }
      }
    }
    ...
    pub fn add(&self, other: &Self) -> Self {
      if self.x == other.x {
        ...
        if self.y == other.y {
          ...
          Self { x: G::Base::ZERO, y: G::Base::ZERO, is_infinity: true }
        }
      } else {
    ...
    pub fn double(&self) -> Self {
      if self.is_infinity {
        return Self { x: G::Base::ZERO, y: G::Base::ZERO, is_infinity: true };
      }
  ...
  pub fn inputize_allocated_point<G: Group, CS: ConstraintSystem<G::Base>>(
    ...
    let _ = p.is_infinity.inputize(cs.namespace(|| "Input point.is_infinity"));
  }
  ...
  where
    ...
    let e_curve = a_curve.mul(C::Scalar::from_repr(s.to_repr()).unwrap()).to_affine();
  ...
  where
    ...
    let (a, e, s) = synthesize_smul::<E1::GE, _>(cs.namespace(|| "synthesize"));
    let (inst, witness) = cs.r1cs_instance_and_witness(&shape, &ck).unwrap();

    let a_p: Point<E1::GE> = Point::new(
      a.x.get_value().unwrap(),
      a.y.get_value().unwrap(),
      a.is_infinity.get_value().unwrap() == <E1 as Engine>::Base::ONE,
    );
    let e_p: Point<E1::GE> = Point::new(
  ...
  where
    ...
    // Then the satisfying assignment
    let mut cs = SatisfyingAssignment::<E2>::new();
    let (a, e) = synthesize_add_equal::<E1::GE, _>(cs.namespace(|| "synthesize add equal"));
    let (inst, witness) = cs.r1cs_instance_and_witness(&shape, &ck).unwrap();
    let a_p: Point<E1::GE> = Point::new(
      a.x.get_value().unwrap(),
      a.y.get_value().unwrap(),
  ...
  where
    ...
    let b = &mut a.clone();
    b.y =
      AllocatedNum::alloc(cs.namespace(|| "allocate negation of a"), || Ok(G::Base::ZERO)).unwrap();
  ...
  fn test_ecc_circuit_add_negation() {
    test_ecc_circuit_add_negation_with::<Bn256EngineIPA, GrumpkinEngine>(&expect!["39"], &expect![
      "34"
    ]);
    test_ecc_circuit_add_negation_with::<GrumpkinEngine, Bn256EngineIPA>(&expect!["39"], &expect![
      "34"
    ]);
  }
    ...
    E2: Engine<Base = <E1 as Engine>::Scalar>,
  {
    ...
    // Then the satisfying assignment
    let mut cs = SatisfyingAssignment::<E2>::new();
    let e = synthesize_add_negation::<E1::GE, _>(cs.namespace(|| "synthesize add negation"));
    let (inst, witness) = cs.r1cs_instance_and_witness(&shape, &ck).unwrap();
    let e_p: Point<E1::GE> = Point::new(
      e.x.get_value().unwrap(),
      e.y.get_value().unwrap(),
