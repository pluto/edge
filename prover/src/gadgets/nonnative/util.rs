...
impl<Scalar: PrimeField> Bit<Scalar> {
  ...
  pub fn alloc<CS: ConstraintSystem<Scalar>>(
    ...
    Ok(Self { bit: LinearCombination::zero() + var, value })
  }
}
...
impl<Scalar: PrimeField> Num<Scalar> {
  ...
  where
    ...
    Ok(Self { value: new_value, num: LinearCombination::zero() + var })
  }
  ...
  pub fn fits_in_bits<CS: ConstraintSystem<Scalar>>(
    ...
    let bits: Vec<Variable> = (1..n_bits)
      .map(|i| {
        cs.alloc(
          || format!("bit {i}"),
          || {
            let r = if *v.grab()?.get_bit(i).grab()? { Scalar::ONE } else { Scalar::ZERO };
            Ok(r)
          },
        )
      })
    ...
    for (i, v) in bits.iter().enumerate() {
      cs.enforce(|| format!("{i} is bit"), |lc| lc + *v, |lc| lc + CS::one() - *v, |lc| lc)
    }
  ...
  pub fn is_equal<CS: ConstraintSystem<Scalar>>(&self, mut cs: CS, other: &Bitvector<Scalar>) {
    let mut f = Scalar::ONE;
    let sum = other.allocations.iter().fold(LinearCombination::zero(), |lc, bit| {
      let l = lc + (f, &bit.bit);
      f = f.double();
      l
    });
  ...
  pub fn decompose<CS: ConstraintSystem<Scalar>>(
    ...
    let allocations: Vec<Bit<Scalar>> = (0..n_bits)
      .map(|bit_i| {
        Bit::alloc(cs.namespace(|| format!("bit{bit_i}")), values.as_ref().map(|vs| vs[bit_i]))
      })
      .collect::<Result<Vec<_>, _>>()?;
    let mut f = Scalar::ONE;
    let sum = allocations.iter().fold(LinearCombination::zero(), |lc, bit| {
      let l = lc + (f, &bit.bit);
      f = f.double();
      l
    });
    let sum_lc = LinearCombination::zero() + &self.num - &sum;
    cs.enforce(|| "sum", |lc| lc + &sum_lc, |lc| lc + CS::one(), |lc| lc);
    let bits: Vec<LinearCombination<Scalar>> =
      allocations.iter().map(|a| LinearCombination::zero() + &a.bit).collect();
    Ok(Bitvector { allocations, values, bits })
  }
...
mod tests {
  ...
  // the write_be function above assumes Field::to_repr() outputs a representation
  // that's an instance of `AsRef<[u8]>` in lower endian. We test that here,
  // as this is not what the I2OSP standard recommends and may change in some
  // implementations.
