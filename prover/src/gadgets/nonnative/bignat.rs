...
impl BigNatParams {
  pub fn new(limb_width: usize, n_limbs: usize) -> Self {
    ...
    max_word -= 1;
    Self { max_word, n_limbs, limb_width, min_bits: 0 }
  }
}
...
pub struct BigNat<Scalar: PrimeField> {
  /// The linear combinations which constrain the value of each limb of the
  /// number
...
impl<Scalar: PrimeField> PartialEq for BigNat<Scalar> {
  fn eq(&self, other: &Self) -> bool { self.value == other.value && self.params == other.params }
}
...
impl<Scalar: PrimeField> From<BigNat<Scalar>> for Polynomial<Scalar> {
  fn from(other: BigNat<Scalar>) -> Self {
    Self { coefficients: other.limbs, values: other.limb_values }
  }
}

impl<Scalar: PrimeField> BigNat<Scalar> {
  /// Allocates a `BigNat` in the circuit with `n_limbs` limbs of width
  /// `limb_width` each. If `max_word` is missing, then it is assumed to
  /// be `(2 << limb_width) - 1`. The value is provided by a closure
  /// returning limb values.
  ...
  /// Allocates a `BigNat` in the circuit with `n_limbs` limbs of width
  /// `limb_width` each. The `max_word` is guaranteed to be `(2 <<
  /// limb_width) - 1`. The value is provided by a closure returning a
  /// natural number.
  ...
  /// Allocates a `BigNat` in the circuit with `n_limbs` limbs of width
  /// `limb_width` each. The `max_word` is guaranteed to be `(2 <<
  /// limb_width) - 1`. The value is provided by an allocated number
  ...
  pub fn as_limbs(&self) -> Vec<Num<Scalar>> {
    ...
    for (i, lc) in self.limbs.iter().enumerate() {
      limbs.push(Num::new(self.limb_values.as_ref().map(|vs| vs[i]), lc.clone()));
    }
    limbs
  }
  ...
  pub fn decompose<CS: ConstraintSystem<Scalar>>(
    ...
    let bitvectors: Vec<Bitvector<Scalar>> = self
      ...
      .map(|(i, (limb, limb_value))| {
        Num::new(limb_value, limb.clone())
          .decompose(cs.namespace(|| format!("subdecmop {i}")), self.params.limb_width)
      })
      .collect::<Result<Vec<_>, _>>()?;
    ...
    Ok(Bitvector { bits, values, allocations })
  }
  ...
  pub fn equal_when_carried<CS: ConstraintSystem<Scalar>>(
      ...
      cs.enforce(
        ...
        |lc| {
          lc + &carry_in.num + &self.limbs[i] - &other.limbs[i]
            ...
            - (nat_to_f(&(&accumulated_extra % &target_base)).unwrap(), CS::one())
        },
      );
    ...
    for (i, zero_limb) in self.limbs.iter().enumerate().skip(n) {
      cs.enforce(|| format!("zero self {i}"), |lc| lc, |lc| lc, |lc| lc + zero_limb);
    }
    for (i, zero_limb) in other.limbs.iter().enumerate().skip(n) {
      cs.enforce(|| format!("zero other {i}"), |lc| lc, |lc| lc, |lc| lc + zero_limb);
    }
    Ok(())
  }

  /// Constrain `self` to be equal to `other`, after carrying both.
  /// Uses regrouping internally to take full advantage of the field size and
  /// reduce the amount of carrying.
  pub fn equal_when_carried_regroup<CS: ConstraintSystem<Scalar>>(
    ...
    let carry_bits = (((max_word.to_f64().unwrap() * 2.0).log2() - self.params.limb_width as f64)
      .ceil()
      + 0.1) as usize;
    let limbs_per_group = max((Scalar::CAPACITY as usize - carry_bits) / self.params.limb_width, 1);
  ...
  pub fn add(&self, other: &Self) -> Result<Self, SynthesisError> {
    ...
    let value = self.value.as_ref().and_then(|x| other.value.as_ref().map(|y| x + y));
    Ok(Self {
      limb_values,
      value,
      limbs,
...
impl<Scalar: PrimeField> Polynomial<Scalar> {
  pub fn alloc_product<CS: ConstraintSystem<Scalar>>(
    ...
    let values = self.values.as_ref().and_then(|self_vs| {
      other.values.as_ref().map(|other_vs| {
        let mut values: Vec<Scalar> =
          std::iter::repeat_with(|| Scalar::ZERO).take(n_product_coeffs).collect();
    ...
    let product = Self { coefficients, values };
    let one = Scalar::ONE;
