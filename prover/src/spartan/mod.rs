...
//! We provide two implementations, one in snark.rs (which does not use any
//! preprocessing) and another in ppsnark.rs (which uses preprocessing to keep
//! the verifier's state small if the PCS provides a succinct verifier)
//! We also provide direct.rs that allows proving a step circuit directly with
//! either of the two SNARKs.
//!
//! In polynomial.rs we also provide foundational types and functions for
//! manipulating multilinear polynomials.
...
impl<E: Engine> PolyEvalWitness<E> {
  ...
  /// # Details
  ///
  /// We allow the input polynomials to have different sizes, and interpret
  /// smaller ones as being padded with 0 to the maximum size of all
  /// polynomials.
  fn batch_diff_size(W: &[&Self], s: E::Scalar) -> Self {
    ...
    let p = zip_with!((p_vec, powers.par_iter()), |v, weight| {
      ...
      v.iter().map(|&x| if *weight != E::Scalar::ONE { x * *weight } else { x }).collect::<Vec<_>>()
    })
    .reduce(
      || vec![E::Scalar::ZERO; size_max],
      |left, right| {
        // Sum into the largest polynomial
        let (mut big, small) = if left.len() > right.len() { (left, right) } else { (right, left) };

        #[allow(clippy::disallowed_methods)]
        big.par_iter_mut().zip(small.par_iter()).for_each(|(b, s)| *b += s);
  ...
  /// Given a set of polynomials \[Pᵢ\] and a scalar `s`, this method computes
  /// the weighted sum of the polynomials, where each polynomial Pᵢ is
  /// scaled by sⁱ.
  ///
  /// # Panics
  ///
  /// This method panics if the polynomials in `p_vec` are not all of the same
  /// length.
  fn batch(p_vec: &[&Vec<E::Scalar>], s: &E::Scalar) -> Self {
    p_vec.iter().skip(1).for_each(|p| assert_eq!(p.len(), p_vec[0].len()));
...
impl<E: Engine> PolyEvalInstance<E> {
  fn batch_diff_size(
    ...
    let evals_scaled = zip_with!(iter, (e_vec, num_vars), |eval, num_rounds| {
      ...
      // Compute L₀(x_lo)
      let lagrange_eval = r_lo.iter().map(|r| E::Scalar::ONE - r).product::<E::Scalar>();
    ...
    Self { c: comm_joint, x, e: eval_joint }
  }
