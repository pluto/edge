//! This module implements `BatchedRelaxedR1CSSNARKTrait` using Spartan that is
//! generic over the polynomial commitment and evaluation argument (i.e., a PCS)
...
//! verifier when using an IPA-based polynomial commitment scheme. This batched
//! implementation batches the outer and inner sumchecks of the Spartan SNARK.
...
/// A succinct proof of knowledge of a witness to a batch of relaxed R1CS
/// instances The proof is produced using Spartan's combination of the sum-check
/// and the commitment to a vector viewed as a polynomial commitment
...
impl<E: Engine, EE: EvaluationEngineTrait<E>> VerifierKey<E, EE> {
  fn new(shapes: Vec<R1CSShape<E>>, vk_ee: EE::VerifierKey) -> Self {
    Self { vk_ee, S: shapes, digest: OnceCell::new() }
  }
}
  ...
  for BatchedRelaxedR1CSSNARK<E, EE>
{
  ...
  fn prove(
    ...
    let polys_Z = zip_with!(iter, (polys_W, U), |w, u| [w.clone(), vec![u.u], u.X.clone()]
      .concat())
    .collect::<Vec<Vec<_>>>();

    let (num_rounds_x, num_rounds_y): (Vec<_>, Vec<_>) =
      S.iter().map(|s| (s.num_cons.log_2(), s.num_vars.log_2() + 1)).unzip();
    ...
    let (sc_proof_outer, r_x, claims_outer) = SumcheckProof::prove_cubic_with_additive_term_batch(
      ...
      &num_rounds_x,
      polys_tau,
      polys_Az.into_iter().map(MultilinearPolynomial::new).collect(),
      polys_Bz.into_iter().map(MultilinearPolynomial::new).collect(),
      polys_uCz_E.into_iter().map(MultilinearPolynomial::new).collect(),
      &outer_r_powers,
      comb_func_outer,
      &mut transcript,
    )?;
    ...
    evals_Az_Bz_Cz.iter().zip_eq(evals_E.iter()).for_each(
      |(&(eval_Az, eval_Bz, eval_Cz), &eval_E)| {
        transcript.absorb(b"claims_outer", &[eval_Az, eval_Bz, eval_Cz, eval_E].as_slice())
      },
    );
    ...
    let polys_ABCs = {
      let inner = |M_evals_As: Vec<E::Scalar>,
       ...
       -> Vec<E::Scalar> {
        zip_with!(into_par_iter, (M_evals_As, M_evals_Bs, M_evals_Cs), |eval_A, eval_B, eval_C| {
          eval_A + inner_r * eval_B + inner_r_square * eval_C
        })
        .collect::<Vec<_>>()
      };
  ...
  fn verify(&self, vk: &Self::VerifierKey, U: &[RelaxedR1CSInstance<E>]) -> Result<(), NovaError> {
    ...
    let (num_rounds_x, num_rounds_y): (Vec<_>, Vec<_>) =
      vk.S.iter().map(|s| (s.num_cons.log_2(), s.num_vars.log_2() + 1)).unzip();
    let num_rounds_x_max = *num_rounds_x.iter().max().unwrap();
    let num_rounds_y_max = *num_rounds_y.iter().max().unwrap();
    ...
    // The evaluation point for each instance is therefore r_x[num_rounds_x_max -
    // num_rounds_x..]
    let r_x = num_rounds_x
      .iter()
      .map(|num_rounds| r_x[(num_rounds_x_max - num_rounds)..].to_vec())
    ...
    let chis_r_x =
      r_x.par_iter().map(|r_x| EqPolynomial::evals_from_points(r_x)).collect::<Vec<_>>();
    ...
    let multi_evaluate = |M_vec: &[&SparseMatrix<E::Scalar>],
     ...
     -> Vec<E::Scalar> {
      ...
      M_vec.par_iter().map(|&M_vec| evaluate_with_table(M_vec, T_x, &T_y)).collect()
    };

    // Compute inner claim ∑ᵢ r³ⁱ⋅(Aᵢ(r_x, r_y) + r⋅Bᵢ(r_x, r_y) + r²⋅Cᵢ(r_x,
    // r_y))⋅Zᵢ(r_y)
    ...
    let u_vec = {
      ...
      u_vec.extend(zip_with!(iter, (self.evals_E, U, r_x), |eval, u, r_x| {
        PolyEvalInstance { c: u.comm_E, x: r_x.to_vec(), e: *eval }
      }));
      u_vec
    };
