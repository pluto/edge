...
impl<E: Engine> SumcheckProof<E> {
    ...
    F: Fn(&E::Scalar, &E::Scalar) -> E::Scalar + Sync,
  {
    ...
    (0..len)
      ...
      .reduce(|| (E::Scalar::ZERO, E::Scalar::ZERO), |a, b| (a.0 + b.0, a.1 + b.1))
  }
    ...
    F: Fn(&E::Scalar, &E::Scalar) -> E::Scalar + Sync,
  {
    ...
    for _ in 0..num_rounds {
      ...
      rayon::join(|| poly_A.bind_poly_var_top(&r_i), || poly_B.bind_poly_var_top(&r_i));
    }

    Ok((Self { compressed_polys: polys }, r, vec![poly_A[0], poly_B[0]]))
  }
    ...
    F: Fn(&E::Scalar, &E::Scalar) -> E::Scalar + Sync,
  {
    ...
    for (i, &num_rounds) in num_rounds.iter().enumerate() {
      ...
      for (l, polyname) in [(a.len(), "poly_A_vec"), (b.len(), "poly_B_vec")].iter() {
        assert_eq!(*l, expected_size, "Mismatch in size for {} at index {}", polyname, i);
      }
    }

    let num_rounds_max = *num_rounds.iter().max().unwrap();
    let mut e = zip_with!(iter, (claims, num_rounds, coeffs), |claim, num_rounds, coeff| {
    ...
    for current_round in 0..num_rounds_max {
      ...
      zip_with_for_each!(
        (num_rounds.par_iter(), poly_A_vec.par_iter_mut(), poly_B_vec.par_iter_mut()),
    ...
    let poly_A_final = poly_A_vec.into_iter().map(|poly| poly[0]).collect::<Vec<_>>();
    let poly_B_final = poly_B_vec.into_iter().map(|poly| poly[0]).collect::<Vec<_>>();

    let eval_expected =
      zip_with!(iter, (poly_A_final, poly_B_final, coeffs), |eA, eB, coeff| comb_func(eA, eB)
        * coeff)
      .sum::<E::Scalar>();
    ...
    F: Fn(&E::Scalar, &E::Scalar, &E::Scalar) -> E::Scalar + Sync,
  {
    ...
    (0..len)
      ...
      .map(|i| {
        ...
        let eval_point_2 = comb_func(&poly_A_bound_point, &poly_B_bound_point, &poly_C_bound_point);

        // eval 3: bound_func is -2A(low) + 3A(high); computed incrementally with
        // bound_func applied to eval(2)
        ...
        let eval_point_3 = comb_func(&poly_A_bound_point, &poly_B_bound_point, &poly_C_bound_point);
    ...
    F: Fn(&E::Scalar, &E::Scalar, &E::Scalar, &E::Scalar) -> E::Scalar + Sync,
  {
    ...
    for _ in 0..num_rounds {
      ...
      rayon::join(
        || rayon::join(|| poly_A.bind_poly_var_top(&r_i), || poly_B.bind_poly_var_top(&r_i)),
        || rayon::join(|| poly_C.bind_poly_var_top(&r_i), || poly_D.bind_poly_var_top(&r_i)),
      );
    }
    ...
    F: Fn(&E::Scalar, &E::Scalar, &E::Scalar, &E::Scalar) -> E::Scalar + Sync,
  {
    ...
    for (i, &num_rounds) in num_rounds.iter().enumerate() {
      ...
      for (l, polyname) in
        [(a.len(), "poly_A"), (b.len(), "poly_B"), (c.len(), "poly_C"), (d.len(), "poly_D")].iter()
      {
        assert_eq!(*l, expected_size, "Mismatch in size for {} at index {}", polyname, i);
      }
    }
    ...
    let mut claim_per_round =
      zip_with!(iter, (claims, num_rounds, coeffs), |claim, num_rounds, coeff| {
        let scaled_claim = E::Scalar::from((1 << (num_rounds_max - num_rounds)) as u64) * claim;
        scaled_claim * *coeff
      })
      .sum();
    ...
    Ok((Self { compressed_polys: polys }, r, vec![
      poly_A_final,
      poly_B_final,
      poly_C_final,
      poly_D_final,
