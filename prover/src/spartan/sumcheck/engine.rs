...
pub trait SumcheckEngine<E: Engine>: Send + Sync {
  ...
  /// returns evaluation points at 0, 2, d-1 (where d is the degree of the
...
/// The [`WitnessBoundSumcheck`] ensures that the witness polynomial W defined
/// over n = log(N) variables, is zero outside of the first `num_vars = 2^m`
/// entries.
///
/// # Details
///
/// The `W` polynomial is padded with zeros to size N = 2^n.
/// The `masked_eq` polynomials is defined as with regards to a random challenge
/// `tau` as the eq(tau) polynomial, where the first 2^m evaluations to 0.
///
...
impl<E: Engine> SumcheckEngine<E> for WitnessBoundSumcheck<E> {
  fn initial_claims(&self) -> Vec<E::Scalar> { vec![E::Scalar::ZERO] }
...
impl<E: Engine> MemorySumcheckInstance<E> {
  ...
  /// and
  ///   `TS_row`, `TS_col` are integer-valued vectors representing the number
  /// of reads   to each memory cell of `L_row`, `L_col`
  ///
  /// The function returns oracles for the polynomials TS[i]/(T[i] + r),
  /// 1/(W[i] + r), as well as auxiliary polynomials T[i] + r, W[i] + r
  pub fn compute_oracles(
    ...
    let hash_func_vec = |mem: &[E::Scalar],
     ...
     -> (Vec<E::Scalar>, Vec<E::Scalar>) {
      ...
      rayon::join(
        ...
        || (0..addr.len()).map(|i| hash_func(&addr[i], &lookups[i])).collect::<Vec<E::Scalar>>(),
      )
    };
    ...
    let helper = |T: &[E::Scalar],
                  ...
                  r: &E::Scalar|
     -> (
      (Result<Vec<E::Scalar>, NovaError>, Result<Vec<E::Scalar>, NovaError>),
      (Vec<E::Scalar>, Vec<E::Scalar>),
    ) {
      rayon::join(
        || {
    ...
    ) = rayon::join(|| helper(&T_row, &W_row, ts_row, r), || helper(&T_col, &W_col, ts_col, r));
    ...
    let poly_vec = [t_plus_r_inv_row, w_plus_r_inv_row, t_plus_r_inv_col, w_plus_r_inv_col];
...
impl<E: Engine> SumcheckEngine<E> for MemorySumcheckInstance<E> {
  fn initial_claims(&self) -> Vec<E::Scalar> { vec![E::Scalar::ZERO; 6] }
  ...
  fn final_claims(&self) -> Vec<Vec<E::Scalar>> {
    let poly_row_final = vec![self.t_plus_r_inv_row[0], self.w_plus_r_inv_row[0], self.ts_row[0]];

    let poly_col_final = vec![self.t_plus_r_inv_col[0], self.w_plus_r_inv_col[0], self.ts_col[0]];
...
impl<E: Engine> SumcheckEngine<E> for OuterSumcheckInstance<E> {
  fn initial_claims(&self) -> Vec<E::Scalar> { vec![E::Scalar::ZERO, self.eval_Mz_at_tau] }
  ...
  fn evaluation_points(&self) -> Vec<Vec<E::Scalar>> {
    ...
    vec![vec![eval_point_h_0, eval_point_h_2, eval_point_h_3], vec![
      eval_point_e_0,
      eval_point_e_2,
      eval_point_e_3,
    ]]
  ...
  fn final_claims(&self) -> Vec<Vec<E::Scalar>> { vec![vec![self.poly_Az[0], self.poly_Bz[0]]] }
}
...
impl<E: Engine> InnerSumcheckInstance<E> {
  pub fn new(
    ...
    Self { claim, poly_L_row, poly_L_col, poly_val }
  }
}
impl<E: Engine> SumcheckEngine<E> for InnerSumcheckInstance<E> {
  fn initial_claims(&self) -> Vec<E::Scalar> { vec![self.claim] }
  ...
  fn bound(&mut self, r: &E::Scalar) {
    [&mut self.poly_L_row, &mut self.poly_L_col, &mut self.poly_val]
      .par_iter_mut()
      .for_each(|poly| poly.bind_poly_var_top(r));
  }
