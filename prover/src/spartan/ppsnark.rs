//! This module implements `RelaxedR1CSSNARK` traits using a spark-based
//! approach to prove evaluations of sparse multilinear polynomials involved in
//! Spartan's sum-check protocol, thereby providing a preprocessing SNARK
//! The verifier in this preprocessing SNARK maintains a commitment to R1CS
//! matrices. This is beneficial when using a polynomial commitment scheme in
//! which the verifier's costs is succinct. This code includes experimental
//! optimizations to reduce runtimes and proof sizes.
use core::cmp::max;
use std::sync::Arc;
...
impl<E: Engine> R1CSShapeSparkRepr<E> {
  /// represents `R1CSShape` in a Spark-friendly format amenable to memory
  /// checking
  pub fn new(S: &R1CSShape<E>) -> Self {
    ...
    let to_vec_scalar = |v: &[usize]| -> Vec<E::Scalar> {
      v.iter().map(|x| E::Scalar::from(*x as u64)).collect::<Vec<_>>()
    };
  ...
  pub(in crate::spartan) fn commit(&self, ck: &CommitmentKey<E>) -> R1CSShapeSparkCommitment<E> {
    let comm_vec: Vec<Commitment<E>> =
      [&self.row, &self.col, &self.val_A, &self.val_B, &self.val_C, &self.ts_row, &self.ts_col]
        .par_iter()
  ...
  fn evaluation_oracles(
    ...
    r_x: &E::Scalar,
    z: &[E::Scalar],
  ) -> (Vec<E::Scalar>, Vec<E::Scalar>, Vec<E::Scalar>, Vec<E::Scalar>) {
...
impl<E: Engine, EE: EvaluationEngineTrait<E>> SimpleDigestible for VerifierKey<E, EE> where EE::VerifierKey: Serialize {}
...
impl<E: Engine, EE: EvaluationEngineTrait<E>> RelaxedR1CSSNARK<E, EE> {
    ...
    T4: SumcheckEngine<E>,
  {
    ...
    for _ in 0..num_rounds {
      ...
      let evals = vec![evals_combined_0, e - evals_combined_0, evals_combined_2, evals_combined_3];
    ...
    Ok((SumcheckProof::new(cubic_polys), r, mem_claims, outer_claims, inner_claims, witness_claims))
  }
}
...
impl<E: Engine, EE: EvaluationEngineTrait<E>> VerifierKey<E, EE> {
  fn new(
    ...
    Self { num_cons, num_vars, S_comm, vk_ee, digest: Default::default() }
  }
}
...
impl<E: Engine, EE: EvaluationEngineTrait<E>> RelaxedR1CSSNARKTrait<E> for RelaxedR1CSSNARK<E, EE> {
  ...
  fn prove(
    ...
    // append the verifier key (which includes commitment to R1CS matrices) and the
    // RelaxedR1CSInstance to the transcript
    ...
    // (1) 0 = \sum_x poly_tau(x) * (poly_Az(x) * poly_Bz(x) - poly_uCz_E(x)), and
    // eval_Az_at_tau + r * eval_Bz_at_tau + r^2 * eval_Cz_at_tau =
    // (Az+r*Bz+r^2*Cz)(tau) (2) eval_Az_at_tau + c * eval_Bz_at_tau + c^2 *
    // eval_Cz_at_tau = \sum_y L_row(y) * (val_A(y) + c * val_B(y) + c^2 * val_C(y))
    // * L_col(y) (3) L_row(i) = eq(tau, row(i)) and L_col(i) = z(col(i))
    // (4) Check that the witness polynomial W is well-formed e.g., it is padded
    // with only zeros
    ...
    let ((mut outer_sc_inst, mut inner_sc_inst), mem_res) = rayon::join(
      || {
        ...
        let outer_sc_inst = OuterSumcheckInstance::new(
          ...
          Az.clone(),
          Bz.clone(),
          (0..Cz.len()).map(|i| U.u * Cz[i] + E[i]).collect::<Vec<E::Scalar>>(),
      ...
      || {
        ...
        // hash the tuples of (addr,val) memory contents and read responses into a
        // single field element using `hash_func`
    ...
    // compute the remaining claims that did not come for free from the sum-check
    // prover
  ...
  fn verify(&self, vk: &Self::VerifierKey, U: &RelaxedR1CSInstance<E>) -> Result<(), NovaError> {
    ...
    // append the verifier key (including commitment to R1CS matrices) and the
    // RelaxedR1CSInstance to the transcript
    ...
    // since all the three polynomials are opened at tau,
    // we can combine them into a single polynomial opened at tau
    let eval_vec = vec![self.eval_Az_at_tau, self.eval_Bz_at_tau, self.eval_Cz_at_tau];
    ...
    let claim_sc_final_expected = {
      ...
      let eval_t_plus_r_col = {
        ...
        let eval_val_col = {
          ...
          let eval_X = {
            // public IO is (u, X)
            let X = vec![U.u].into_iter().chain(U.X.iter().cloned()).collect::<Vec<E::Scalar>>();
    ...
    // verify
    EE::verify(&vk.vk_ee, &mut transcript, &u.c, &rand_sc, &u.e, &self.eval_arg)?;
