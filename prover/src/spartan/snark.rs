...
//! This version of Spartan does not use preprocessing so the verifier keeps the
//! entire description of R1CS matrices. This is essentially optimal for the
//! verifier when using an IPA-based polynomial commitment scheme.
...
impl<E: Engine, EE: EvaluationEngineTrait<E>> VerifierKey<E, EE> {
  fn new(shape: R1CSShape<E>, vk_ee: EE::VerifierKey) -> Self {
    Self { vk_ee, S: shape, digest: OnceCell::new() }
  }
}
...
impl<E: Engine, EE: EvaluationEngineTrait<E>> RelaxedR1CSSNARKTrait<E> for RelaxedR1CSSNARK<E, EE> {
  ...
  fn setup(
    ...
    let pk = ProverKey { pk_ee, vk_digest: vk.digest() };
  ...
  fn prove(
    ...
    transcript.absorb(b"claims_outer", &[claim_Az, claim_Bz, claim_Cz, eval_E].as_slice());
    ...
    // We will reduce a vector of claims of evaluations at different points into
    // claims about them at the same point. For example, eval_W =?
    // W(r_y[1..]) and eval_E =? E(r_x) into two claims: eval_W_prime =?
    // W(rz) and eval_E_prime =? E(rz) We can them combine the two into one:
    // eval_W_prime + gamma * eval_E_prime =? (W + gamma*E)(rz), where gamma
    // is a public challenge Since commitments to W and E are homomorphic,
    // the verifier can compute a commitment to the batched polynomial.
    let eval_W = MultilinearPolynomial::evaluate_with(&W.W, &r_y[1..]);

    let w_vec = vec![PolyEvalWitness { p: W.W }, PolyEvalWitness { p: W.E }];
    let u_vec =
      vec![PolyEvalInstance { c: U.comm_W, x: r_y[1..].to_vec(), e: eval_W }, PolyEvalInstance {
        c: U.comm_E,
        x: r_x,
        e: eval_E,
      }];
  ...
  fn verify(&self, vk: &Self::VerifierKey, U: &RelaxedR1CSInstance<E>) -> Result<(), NovaError> {
    ...
    // append the digest of R1CS matrices and the RelaxedR1CSInstance to the
    // transcript
    ...
    let (claim_outer_final, r_x) =
      self.sc_proof_outer.verify(E::Scalar::ZERO, num_rounds_x, 3, &mut transcript)?;
    ...
    transcript.absorb(
      b"claims_outer",
      &[self.claims_outer.0, self.claims_outer.1, self.claims_outer.2, self.eval_E].as_slice(),
    );
    ...
    let (claim_inner_final, r_y) =
      self.sc_proof_inner.verify(claim_inner_joint, num_rounds_y, 2, &mut transcript)?;
    ...
    let multi_evaluate = |M_vec: &[&SparseMatrix<E::Scalar>],
     ...
     -> Vec<E::Scalar> {
      ...
      (0..M_vec.len()).into_par_iter().map(|i| evaluate_with_table(M_vec[i], &T_x, &T_y)).collect()
    };
    ...
    let u_vec: Vec<PolyEvalInstance<E>> = vec![
      PolyEvalInstance { c: U.comm_W, x: r_y[1..].to_vec(), e: self.eval_W },
      PolyEvalInstance { c: U.comm_E, x: r_x, e: self.eval_E },
    ];

    let batched_u =
      batch_eval_verify(u_vec, &mut transcript, &self.sc_proof_batch, &self.evals_batch)?;
...
pub(in crate::spartan) fn batch_eval_reduce<E: Engine>(
  ...
  transcript: &mut E::TE,
) -> Result<(PolyEvalInstance<E>, PolyEvalWitness<E>, SumcheckProof<E>, Vec<E::Scalar>), NovaError>
{
  ...
  zip_with_for_each!(iter, (w_vec, num_rounds), |w, num_vars| assert_eq!(w.p.len(), 1 << num_vars));
  ...
  // Pᵢ(X)
  let polys_P: Vec<MultilinearPolynomial<E::Scalar>> =
    w_vec.iter().map(|w| MultilinearPolynomial::new(w.p.clone())).collect();
  // eq(xᵢ, X)
...
pub(in crate::spartan) fn batch_eval_verify<E: Engine>(
  ...
  let claim_batch_final_expected = {
    ...
    zip_with!((evals_r, evals_batch.iter(), powers_of_rho.iter()), |e_i, p_i, rho_i| e_i
      * *p_i
      * rho_i)
    .sum()
  };
