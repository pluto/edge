...
/// polynomial $pow(x,t)$, denoted as $\tilde{pow}(x, t)$.
///
...
impl<Scalar: PrimeField> PowPolynomial<Scalar> {
  ...
  pub fn new(t: &Scalar, ell: usize) -> Self {
    ...
    Self { eq: EqPolynomial::new(t_pow) }
  }

  /// Create powers the following powers of `t`:
  /// [t^{2^0}, t^{2^1}, ..., t^{2^{ell-1}}]
  pub fn squares(t: &Scalar, ell: usize) -> Vec<Scalar> {
    successors(Some(*t), |p: &Scalar| Some(p.square())).take(ell).collect::<Vec<_>>()
  }

  /// Creates the evals corresponding to a `PowPolynomial` from an
  /// already-existing vector of powers. `t_pow.len() > ell` must be true.
  ...
  /// internal vector `t_pow`.
  ///
  /// Panics if `rx` and `t_pow` have different lengths.
  pub fn evaluate(&self, rx: &[Scalar]) -> Scalar { self.eq.evaluate(rx) }

  pub fn coordinates(self) -> Vec<Scalar> { self.eq.r }

  /// Evaluates the `PowPolynomial` at all the `2^|t_pow|` points in its
  /// domain.
  ///
  /// Returns a vector of Scalars, each corresponding to the polynomial
  /// evaluation at a specific point.
  pub fn evals(&self) -> Vec<Scalar> { self.eq.evals() }
}

impl<Scalar: PrimeField> From<PowPolynomial<Scalar>> for EqPolynomial<Scalar> {
  fn from(polynomial: PowPolynomial<Scalar>) -> Self { polynomial.eq }
}
