...
/// polynomial $eqₘ(x,r)$ over n variables, where the first 2^m evaluations are
/// 0.
///
/// The polynomial is defined by the formula:
/// eqₘ(x,r) = eq(x,r) - ( ∏_{0 ≤ i < n-m} (1−rᵢ)(1−xᵢ) )⋅( ∏_{n-m ≤ i < n}
/// (1−rᵢ)(1−xᵢ) + rᵢ⋅xᵢ )
#[derive(Debug)]
...
impl<'a, Scalar: PrimeField> MaskedEqPolynomial<'a, Scalar> {
  /// Creates a new `MaskedEqPolynomial` from a vector of Scalars `r` of size
  /// n, with the number of masked variables m = `num_masked_vars`.
  pub const fn new(eq: &'a EqPolynomial<Scalar>, num_masked_vars: usize) -> Self {
    MaskedEqPolynomial { eq, num_masked_vars }
  }

  /// Evaluates the `MaskedEqPolynomial` at a given point `rx`.
  ///
  /// This function computes the value of the polynomial at the point
  /// specified by `rx`. It expects `rx` to have the same length as the
  /// internal vector `r`.
  ///
  ...
  pub fn evaluate(&self, rx: &[Scalar]) -> Scalar {
    ...
    let eq_hi = zip_eq(r_hi, rx_hi)
      ...
      .product::<Scalar>();
    let mask_lo =
      zip_eq(r_lo, rx_lo).map(|(r, rx)| (Scalar::ONE - r) * (Scalar::ONE - rx)).product::<Scalar>();

    (eq_lo - mask_lo) * eq_hi
  }

  /// Evaluates the `MaskedEqPolynomial` at all the `2^|r|` points in its
  /// domain.
  ///
  /// Returns a vector of Scalars, each corresponding to the polynomial
  /// evaluation at a specific point.
  pub fn evals(&self) -> Vec<Scalar> { Self::evals_from_points(&self.eq.r, self.num_masked_vars) }

  /// Evaluates the `MaskedEqPolynomial` from the `2^|r|` points in its
  /// domain, without creating an intermediate polynomial representation.
  ///
  /// Returns a vector of Scalars, each corresponding to the polynomial
  /// evaluation at a specific point.
  fn evals_from_points(r: &[Scalar], num_masked_vars: usize) -> Vec<Scalar> {
    ...
    evals[..num_masked_evals].iter_mut().for_each(|e| *e = Scalar::ZERO);
...
mod tests {
  ...
  use super::*;
  use crate::{provider, spartan::polys::eq::EqPolynomial};
  ...
  fn test_masked_eq_polynomial_with<F: PrimeField, R: RngCore + CryptoRng>(
    ...
    // random point
    let r = std::iter::from_fn(|| Some(F::random(&mut rng))).take(num_vars).collect::<Vec<_>>();
    // evaluation point
    let rx = std::iter::from_fn(|| Some(F::random(&mut rng))).take(num_vars).collect::<Vec<_>>();
    ...
    assert_eq!(masked_eq_poly_evals[..num_masked_evals], vec![F::ZERO; num_masked_evals]);
    // ensure the remaining evaluations match eq(r)
    assert_eq!(masked_eq_poly_evals[num_masked_evals..], poly_eq_evals[num_masked_evals..]);
    ...
    let expected_masked_eq_eval =
      zip_eq(rx_evals, masked_eq_poly_evals).map(|(rx, r)| rx * r).sum();
