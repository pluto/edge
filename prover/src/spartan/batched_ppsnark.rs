...
impl<E: Engine, EE: EvaluationEngineTrait<E>> VerifierKey<E, EE> {
  fn new(
    ...
    Self { num_vars, S_comm, vk_ee, digest: Default::default() }
  }
}
  ...
  for BatchedRelaxedR1CSSNARK<E, EE>
{
  ...
  fn setup(
    ...
    let S_comm = S_repr.iter().map(|s_repr| s_repr.commit(&*ck)).collect::<Vec<_>>();
    let num_vars = S.iter().map(|s| s.num_vars).collect::<Vec<_>>();
    let vk = VerifierKey::new(num_vars, S_comm.clone(), vk_ee);
    let pk = ProverKey { pk_ee, S_repr, S_comm, vk_digest: vk.digest() };
    Ok((pk, vk))
  }
  ...
  fn prove(
    ...
    let comms_Az_Bz_Cz = polys_Az_Bz_Cz
      ...
      .collect::<Vec<_>>();
    comms_Az_Bz_Cz.iter().for_each(|comms| transcript.absorb(b"c", &comms.as_slice()));
    ...
    // Pad [Az, Bz, Cz] to Ni
    polys_Az_Bz_Cz.par_iter_mut().zip_eq(Nis.par_iter()).for_each(|(az_bz_cz, &Ni)| {
      az_bz_cz.par_iter_mut().for_each(|mz| mz.resize(Ni, E::Scalar::ZERO))
    });

    // Evaluate and commit to [Az(tau), Bz(tau), Cz(tau)]
    let evals_Az_Bz_Cz_at_tau =
      zip_with!(par_iter, (polys_Az_Bz_Cz, coords_tau), |ABCs, tau_coords| {
        let [Az, Bz, Cz] = ABCs;
    ...
    let polys_L_row_col =
      zip_with!(par_iter, (S, Nis, polys_Z, polys_tau), |S, Ni, poly_Z, poly_tau| {
    ...
    let inner_sc_inst =
      zip_with!(par_iter, (pk.S_repr, evals_Mz, polys_L_row_col), |s_repr, eval_Mz, poly_L| {
        let [poly_L_row, poly_L_col] = poly_L;
        let c_square = c.square();
        let val =
          zip_with!(par_iter, (s_repr.val_A, s_repr.val_B, s_repr.val_C), |v_a, v_b, v_c| *v_a
            + c * *v_b
            + c_square * *v_c)
          .collect::<Vec<_>>();
    ...
    let (evals_Az_Bz_Cz_W_E, evals_L_row_col, evals_mem_oracle, evals_mem_preprocessed) = {
      let evals_Az_Bz =
        claims_outer.into_iter().map(|claims| [claims[0][0], claims[0][1]]).collect::<Vec<_>>();
      ...
      let evals_W = claims_witness.into_iter().map(|claims| claims[0][0]).collect::<Vec<_>>();

      let (evals_Cz_E, evals_mem_val_row_col): (Vec<_>, Vec<_>) =
        zip_with!(iter, (polys_Az_Bz_Cz, polys_E, pk.S_repr), |ABCzs, poly_E, s_repr| {
          let [_, _, Cz] = ABCzs;
          ...
          let e =
            [Cz, poly_E, &s_repr.val_A, &s_repr.val_B, &s_repr.val_C, &s_repr.row, &s_repr.col]
              .into_iter()
              .map(|p| {
      ...
      let evals_Az_Bz_Cz_W_E =
        zip_with!((evals_Az_Bz.into_iter(), evals_Cz_E.into_iter(), evals_W), |Az_Bz, Cz_E, W| {
          let [Az, Bz] = Az_Bz;
          let [Cz, E] = Cz_E;
          [Az, Bz, Cz, W, E]
        })
      ...
      .collect::<Vec<_>>();
      (evals_Az_Bz_Cz_W_E, evals_L_row_col, evals_mem_oracle, evals_mem_preprocessed)
    };
    ...
    let evals_vec = zip_with!(
      ...
      |Az_Bz_Cz_W_E, L_row_col, mem_oracles, mem_preprocessed| {
        chain![Az_Bz_Cz_W_E, L_row_col, mem_oracles, mem_preprocessed].cloned().collect::<Vec<_>>()
      }
    )
    .collect::<Vec<_>>();
    ...
    let comms_vec = zip_with!(
      iter,
      (comms_Az_Bz_Cz, comms_W_E, comms_L_row_col, comms_mem_oracles, pk.S_comm),
      |Az_Bz_Cz, comms_W_E, L_row_col, mem_oracles, S_comm| {
        chain![Az_Bz_Cz, comms_W_E, L_row_col, mem_oracles, [
          &S_comm.comm_val_A,
          &S_comm.comm_val_B,
          &S_comm.comm_val_C,
          &S_comm.comm_row,
    ...
    let w_vec = zip_with!(
      ...
      |Az_Bz_Cz, W, E, L_row_col, mem_oracles, S_repr| {
        chain![Az_Bz_Cz, [W, E], L_row_col, mem_oracles, [
          S_repr.val_A.clone(),
          S_repr.val_B.clone(),
          S_repr.val_C.clone(),
          S_repr.row.clone(),
    ...
    let eval_arg =
      EE::prove(ck, &pk.pk_ee, &mut transcript, &u_batch.c, &w_batch.p, &u_batch.x, &u_batch.e)?;

    let comms_Az_Bz_Cz =
      comms_Az_Bz_Cz.into_iter().map(|comms| comms.map(|comm| comm.compress())).collect();
    let comms_L_row_col =
      comms_L_row_col.into_iter().map(|comms| comms.map(|comm| comm.compress())).collect();
    let comms_mem_oracles =
      comms_mem_oracles.into_iter().map(|comms| comms.map(|comm| comm.compress())).collect();
  ...
  fn verify(&self, vk: &Self::VerifierKey, U: &[RelaxedR1CSInstance<E>]) -> Result<(), NovaError> {
    ...
    let comms_Az_Bz_Cz = self
      .comms_Az_Bz_Cz
      .iter()
      .map(|comms| comms.iter().map(Commitment::<E>::decompress).collect::<Result<Vec<_>, _>>())
    ...
    let (claim_sc_final, rand_sc) = {
      ...
      self.sc.verify_batch(&claims, &num_rounds_by_claim, &s_powers, 3, &mut transcript)?
    };
       ...
       eval_mem_preprocessed| {
        ...
        let Z = {
          ...
          let (factor, rand_sc_unpad) = {
            ...
            let factor =
              rand_sc_lo.iter().fold(E::Scalar::ONE, |acc, r_p| acc * (E::Scalar::ONE - r_p));
        ...
        let claims_outer = [eq_tau * (Az * Bz - U.u * Cz - E), eq_tau * (Az + c * Bz + c * c * Cz)];
    ...
    let u = {
      ...
      let comms_vec = zip_with!(
        ...
        |Az_Bz_Cz, U, L_row_col, mem_oracles, S_comm| {
          chain![Az_Bz_Cz, [U.comm_W, U.comm_E], L_row_col, mem_oracles, [
            S_comm.comm_val_A,
            S_comm.comm_val_B,
            S_comm.comm_val_C,
            S_comm.comm_row,
...
impl<E: Engine, EE: EvaluationEngineTrait<E>> BatchedRelaxedR1CSSNARK<E, EE> {
    ...
    T4: SumcheckEngine<E>,
  {
    ...
    let claims = zip_with!(iter, (mem, outer, inner, witness), |mem, outer, inner, witness| {
    ...
    for i in 0..num_rounds {
      ...
      let evals =
        zip_with!(par_iter, (mem, outer, inner, witness), |mem, outer, inner, witness| {
    ...
    let claims_witness = witness.into_iter().map(|inst| inst.final_claims()).collect();

    Ok((SumcheckProof::new(cubic_polys), r, claims_outer, claims_inner, claims_mem, claims_witness))
  }
  ...
  fn scaled_claims<T: SumcheckEngine<E>>(inst: &T, remaining_variables: usize) -> Vec<E::Scalar> {
    ...
    inst.initial_claims().iter().map(|claim| scaling * claim).collect()
  }
}
