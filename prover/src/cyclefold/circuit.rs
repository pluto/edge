...
/// A structure containing the CycleFold circuit inputs and implementing the
/// synthesize function
...
impl<E: Engine> Default for CycleFoldCircuit<E> {
  fn default() -> Self {
    ...
    Self { commit_1: None, commit_2: None, scalar: None, poseidon_constants }
  }
}
impl<E: Engine> CycleFoldCircuit<E> {
    ...
    CS: ConstraintSystem<E::Base>,
  {
    ...
    let val =
      poseidon_hash_allocated(cs.namespace(|| "hash point"), preimage, &self.poseidon_constants)?;
...
mod tests {
  ...
  fn test_cyclefold_circuit_sat_with<E: CurveCycleEquipped>() {
    ...
    let v1 =
      (0..5).map(|_| <<Dual<E> as Engine>::Scalar as Field>::random(rng)).collect::<Vec<_>>();
    let v2 =
      (0..5).map(|_| <<Dual<E> as Engine>::Scalar as Field>::random(rng)).collect::<Vec<_>>();
    ...
    let r_bits = r.to_le_bits().into_iter().take(128).collect::<Vec<_>>().try_into().unwrap();
    ...
    circuit.synthesize(cs.namespace(|| "synthesizing witness")).unwrap();
  ...
  #[test]
  fn test_cyclefold_circuit_sat() { test_cyclefold_circuit_sat_with::<Bn256EngineKZG>(); }
}
