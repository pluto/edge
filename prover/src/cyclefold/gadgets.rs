//! This module defines many of the gadgets needed in the primary folding
//! circuit
...
// An allocated version of the R1CS instance obtained from a single cyclefold
// invocation
...
impl<E: Engine> AllocatedCycleFoldInstance<E> {
    ...
    CS: ConstraintSystem<E::Base>,
  {
    ...
    self.X.iter().enumerate().try_for_each(|(io_idx, x)| -> Result<(), SynthesisError> {
...
/// An circuit allocated version of the `FoldingData` coming from a CycleFold
/// invocation.
...
impl<E: Engine> AllocatedCycleFoldData<E> {
  ...
  /// The NIFS verifier which folds the CycleFold instance into a running
    ...
    CS: ConstraintSystem<E::Base>,
  {
    ...
    self.U.absorb_in_ro(cs.namespace(|| "absorb cyclefold running instance"), &mut ro)?;
    // running instance `U` does not need to absorbed since u.X[0] = Hash(params, U,
    // i, z0, zi)
    self.u.absorb_in_ro(cs.namespace(|| "absorb cyclefold instance"), &mut ro)?;
    ...
    let r_bn =
      BigNat::from_num(cs.namespace(|| "allocate r_bn"), &Num::from(r), limb_width, n_limbs)?;
    ...
    Ok(AllocatedRelaxedR1CSInstance { W: W_fold, E: E_fold, u: u_fold, X: X_fold })
  }
}
...
pub mod emulated {
  ...
  where G: Group
  {
      ...
      CS: ConstraintSystem<<G as Group>::Base>,
    {
      let x = BigNat::alloc_from_nat(
        cs.namespace(|| "x"),
        || Ok(f_to_nat(&coords.map_or(<G::Scalar as Field>::ZERO, |val| val.0))),
        limb_width,
        n_limbs,
      )?;
      ...
      let y = BigNat::alloc_from_nat(
        cs.namespace(|| "y"),
        || Ok(f_to_nat(&coords.map_or(<G::Scalar as Field>::ZERO, |val| val.1))),
        limb_width,
        n_limbs,
      )?;
      ...
      CS: ConstraintSystem<G::Base>,
    {
      ...
      let is_infinity_num: AllocatedNum<G::Base> =
        AllocatedNum::alloc(cs.namespace(|| "is_infinity"), || {
          self.is_infinity.get_value().map_or(Err(SynthesisError::AssignmentMissing), |bit| {
            if bit {
              Ok(G::Base::ONE)
            } else {
              Ok(G::Base::ZERO)
  ...
  /// A non-native circuit version of a `RelaxedR1CSInstance`. This is used
  /// for the in-circuit representation of the primary running instance
  ...
  where E: Engine
  {
      ...
      CS: ConstraintSystem<<E as Engine>::Base>,
    {
      ...
      Ok(Self { comm_W, comm_E, u, x0, x1 })
    }

    /// Performs a folding of a primary R1CS instance (`u_W`, `u_x0`,
    /// `u_x1`) into a running `AllocatedEmulRelaxedR1CSInstance`
    /// As the curve operations are performed in the CycleFold circuit and
    ...
    pub fn fold_with_r1cs<CS: ConstraintSystem<<E as Engine>::Base>>(
      ...
      let mut ro = E::ROCircuit::new(
        ro_consts,
        1 + NUM_FE_IN_EMULATED_POINT + 2 + NUM_FE_IN_EMULATED_POINT, /* pp_digest + u.W + u.x +
                                                                      * comm_T */
      );
      ro.absorb(pp_digest);
      ...
      Ok(Self { comm_W: W_new, comm_E: E_new, u: u_fold, x0: x0_fold, x1: x1_fold })
    }
    ...
    where
      CS: ConstraintSystem<<E as Engine>::Base>,
    {
      self.comm_W.absorb_in_ro(cs.namespace(|| "absorb comm_W"), ro)?;
      self.comm_E.absorb_in_ro(cs.namespace(|| "absorb comm_E"), ro)?;
