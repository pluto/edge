...
/// A SNARK that holds the proof of a step of an incremental computation of the
/// primary circuit in the CycleFold folding scheme.
/// The difference of this folding scheme from the Nova NIFS in `src/nifs.rs` is
/// that this
#[derive(Debug)]
...
where
  ...
  E2: Engine<Base = <E1 as Engine>::Scalar>,
{
  /// Takes a relaxed R1CS instance-witness pair (U1, W1) and an R1CS
  ...
  pub fn prove(
    ...
    U2: &R1CSInstance<E1>,
    W2: &R1CSWitness<E1>,
  ) -> Result<(Self, (RelaxedR1CSInstance<E1>, RelaxedR1CSWitness<E1>), E1::Scalar), NovaError> {
    let arity = U1.X.len();
    ...
    let mut ro = E2::RO::new(
      ro_consts.clone(),
      1 + NUM_FE_IN_EMULATED_POINT + arity + NUM_FE_IN_EMULATED_POINT, /* pp_digest + u.W
                                                                        * + u.X + T */
    );
    ...
    Ok((Self { comm_T: comm_T.compress(), _p: PhantomData }, (U, W), r))
  }
}

/// A SNARK that holds the proof of a step of an incremental computation of the
/// CycleFold circuit The difference of this folding scheme from the Nova NIFS
/// in `src/nifs.rs` is that this folding prover and verifier must fold in the
/// `RelaxedR1CSInstance` accumulator because the optimization in the
#[derive(Debug)]
...
impl<E: Engine> CycleFoldNIFS<E> {
  ...
  pub fn prove(
    ...
    let mut ro = E::RO::new(
      ro_consts.clone(),
      46, /* 1 + (3 + 3 + 1 + NIO_CYCLE_FOLD * BN_N_LIMBS) + (3 + NIO_CYCLE_FOLD *
           * BN_N_LIMBS) + 3, // digest + (U) + (u) + T */
    );
    ...
    // NOTE: this must be here because the IO for `U2` does not have the data of the
    // hash of `U1`
    ...
    Ok((Self { comm_T: comm_T.compress() }, (U, W)))
  }
}
