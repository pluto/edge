...
#[non_exhaustive]
pub enum NovaError {
  /// returned if the supplied row or col in (row,col,val) tuple is out of
  /// range
  #[error("InvalidIndex")]
  InvalidIndex,
  /// returned if the step circuit calls inputize or alloc_io in its
  /// synthesize method instead of passing output with the return value
  ...
  UnSat,
  /// returned if the supplied witness is not a satisfying witness to a given
  /// shape and instance, with error constraint index
  #[error("UnSatIndex")]
  UnSatIndex(usize),
  ...
  /// returned when the initial input to an incremental computation differs
  /// from a previously declared arity
  #[error("InvalidInitialInputLength")]
  InvalidInitialInputLength,
  /// returned when the step execution produces an output whose length differs
  /// from a previously declared arity
  #[error("InvalidStepOutputLength")]
  InvalidStepOutputLength,
  /// returned when the transcript engine encounters an overflow of the round
  /// number
  ...
  #[error("DigestError")]
  DigestError,
  /// returned when the prover cannot prove the provided statement due to
  /// completeness error
  #[error("InternalError")]
  InternalError,
}
...
impl From<bellpepper_core::SynthesisError> for NovaError {
  fn from(err: bellpepper_core::SynthesisError) -> Self { Self::SynthesisError(err.to_string()) }
}
