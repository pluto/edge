...
where E::Scalar: PrimeField {
  ...
  pub constraints:
    Vec<(LinearCombination<E::Scalar>, LinearCombination<E::Scalar>, LinearCombination<E::Scalar>)>,
  inputs:          usize,
  aux:             usize,
}
...
impl<E: Engine> ShapeCS<E> {
  ...
  pub fn num_inputs(&self) -> usize { self.inputs }

  /// Returns the number of aux inputs defined for this `ShapeCS`.
  pub fn num_aux(&self) -> usize { self.aux }
}

impl<E: Engine> Default for ShapeCS<E> {
  fn default() -> Self { Self { constraints: vec![], inputs: 1, aux: 0 } }
}
