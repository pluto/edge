...
/// A type for functions that hints commitment key sizing by returning the floor
/// of the number of required generators.
pub type CommitmentKeyHint<E> = dyn Fn(&R1CSShape<E>) -> usize;

/// Generates public parameters for a Rank-1 Constraint System (R1CS).
///
/// This function takes into consideration the shape of the R1CS matrices and a
/// hint function for the number of generators. It returns a `CommitmentKey`.
///
/// # Arguments
///
...
/// Computes the number of generators required for the commitment key
...
impl<E: Engine> R1CSShape<E> {
  /// Create an object of type `R1CSShape` from the explicitly specified R1CS
  /// matrices
  pub fn new(
    num_cons: usize,
    num_vars: usize,
    ...
    Ok(Self { num_cons, num_vars, num_io, A, B, C, digest: OnceCell::new() })
  }

  /// Generate a random [`R1CSShape`] with the specified number of
  /// constraints, variables, and public inputs/outputs.
  pub fn random<R: RngCore + CryptoRng>(
    ...
    Self { num_cons, num_vars, num_io, A, B, C, digest: Default::default() }
  }

  /// Generate a satisfying [`RelaxedR1CSWitness`] and [`RelaxedR1CSInstance`]
  /// for this [`R1CSShape`].
  pub fn random_witness_instance<R: RngCore + CryptoRng>(
    ...
    let W = (0..self.num_vars).map(|_| E::Scalar::random(&mut rng)).collect::<Vec<E::Scalar>>();
    let u = E::Scalar::random(&mut rng);
    let X = (0..self.num_io).map(|_| E::Scalar::random(&mut rng)).collect::<Vec<E::Scalar>>();

    let E = self.compute_E(&W, &u, &X).unwrap();

    let (comm_W, comm_E) =
      rayon::join(|| CE::<E>::commit(commitment_key, &W), || CE::<E>::commit(commitment_key, &E));

    let witness = RelaxedR1CSWitness { W, E };
    let instance = RelaxedR1CSInstance { comm_W, comm_E, u, X };
  ...
  // Checks regularity conditions on the R1CSShape, required in Spartan-class
  // SNARKs Returns false if num_cons or num_vars are not powers of two, or if
  // num_io > num_vars
  #[inline]
  ...
  pub(crate) fn multiply_witness(
    ...
    let (Az, (Bz, Cz)) = rayon::join(
      ...
      || rayon::join(|| self.B.multiply_witness(W, u, X), || self.C.multiply_witness(W, u, X)),
    );
  ...
  fn compute_E(
    ...
    let E = zip_with!((Az.into_par_iter(), Bz.into_par_iter(), Cz.into_par_iter()), |a, b, c| a
      * b
      - c * u)
  ...
  /// Checks if the Relaxed R1CS instance is satisfiable given a witness and
  /// its shape
  pub fn is_sat_relaxed(
    ...
    W.E.par_iter().zip_eq(E.into_par_iter()).enumerate().try_for_each(|(i, (we, e))| {
      if *we != e {
  ...
  pub fn commit_T(
    ...
    let (AZ_1_circ_BZ_2, AZ_2_circ_BZ_1, u_1_cdot_CZ_2, u_2_cdot_CZ_1) =
      tracing::trace_span!("cross terms").in_scope(|| {
        let AZ_1_circ_BZ_2 =
          (0..AZ_1.len()).into_par_iter().map(|i| AZ_1[i] * BZ_2[i]).collect::<Vec<E::Scalar>>();
        let AZ_2_circ_BZ_1 =
          (0..AZ_2.len()).into_par_iter().map(|i| AZ_2[i] * BZ_1[i]).collect::<Vec<E::Scalar>>();
        let u_1_cdot_CZ_2 =
          (0..CZ_2.len()).into_par_iter().map(|i| U1.u * CZ_2[i]).collect::<Vec<E::Scalar>>();
        let u_2_cdot_CZ_1 =
          (0..CZ_1.len()).into_par_iter().map(|i| CZ_1[i]).collect::<Vec<E::Scalar>>();
  ...
  pub fn commit_T_into(
    ...
    let R1CSResult { AZ: AZ_1, BZ: BZ_1, CZ: CZ_1 } = ABC_Z_1;

    tracing::info_span!("AZ_2, BZ_2, CZ_2")
      .in_scope(|| self.multiply_witness_into(&W2.W, &E::Scalar::ONE, &U2.X, ABC_Z_2))?;

    let R1CSResult { AZ: AZ_2, BZ: BZ_2, CZ: CZ_2 } = ABC_Z_2;

    // this doesn't allocate memory but has bad temporal cache locality -- should
...
impl<E: Engine> R1CSWitness<E> {
  ...
  pub fn commit(&self, ck: &CommitmentKey<E>) -> Commitment<E> { CE::<E>::commit(ck, &self.W) }
}
...
impl<E: Engine> RelaxedR1CSWitness<E> {
  ...
  pub fn default(S: &R1CSShape<E>) -> Self {
    Self { W: vec![E::Scalar::ZERO; S.num_vars], E: vec![E::Scalar::ZERO; S.num_cons] }
  }
  ...
  pub fn from_r1cs_witness(S: &R1CSShape<E>, witness: R1CSWitness<E>) -> Self {
    Self { W: witness.W, E: vec![E::Scalar::ZERO; S.num_cons] }
  }
  ...
  pub fn fold_mut(
    ...
    self.W.par_iter_mut().zip_eq(&W2.W).for_each(|(a, b)| *a += *r * *b);
    self.E.par_iter_mut().zip_eq(T).for_each(|(a, b)| *a += *r * *b);
...
impl<E: Engine> RelaxedR1CSInstance<E> {
  /// Produces a default `RelaxedR1CSInstance` given `R1CSGens` and
  /// `R1CSShape`
...
pub fn default_T<E: Engine>(num_cons: usize) -> Vec<E::Scalar> { Vec::with_capacity(num_cons) }
...
pub(crate) mod tests {
  ...
  pub(crate) fn tiny_r1cs<E: Engine>(num_vars: usize) -> R1CSShape<E> {
    ...
    let (num_cons, num_vars, num_io, A, B, C) = {
      ...
      // respectively the input and output. The R1CS for this problem
      // consists of the following constraints: `I0 * I0 - Z0 = 0`
      // `Z0 * I0 - Z1 = 0`
      // `(Z1 + I0) * 1 - Z2 = 0`
      // `(Z2 + 5) * 1 - I1 = 0`

      // Relaxed R1CS is a set of three sparse matrices (A B C), where there is a row
      // for every constraint and a column for every entry in z = (vars,
      // u, inputs) An R1CS instance is satisfiable iff:
  ...
  #[test]
  fn test_pad_tiny_r1cs() { test_pad_tiny_r1cs_with::<Bn256EngineKZG>(); }
  ...
  #[test]
  fn test_random_r1cs() { test_random_r1cs_with::<Bn256EngineIPA>(); }
}
