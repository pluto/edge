...
impl<F: PrimeField> SparseMatrix<F> {
  /// 0x0 empty matrix
  pub fn empty() -> Self { Self { data: vec![], indices: vec![], indptr: vec![0], cols: 0 } }
  ...
  pub fn new(matrix: &[(usize, usize, F)], rows: usize, cols: usize) -> Self {
    ...
    Self { data, indices, indptr, cols }
  }

  /// Samples a new random matrix of size `rows` x `cols` with `num_entries`
  /// non-zero entries.
  pub fn random<R: RngCore + CryptoRng>(
    ...
    let matrix =
      indices.into_iter().map(|(row, col)| (row, col, F::random(&mut rng))).collect::<Vec<_>>();
  ...
  pub fn iter_rows(&self) -> impl Iterator<Item = &RowData> {
    self.indptr.windows(2).map(|ptrs| RowData::ref_cast(ptrs.try_into().unwrap()))
  }

  /// Returns a parallel iterator into the rows
  pub fn par_iter_rows(&self) -> impl IndexedParallelIterator<Item = &RowData> {
    self.indptr.par_windows(2).map(|ptrs| RowData::ref_cast(ptrs.try_into().unwrap()))
  }

  /// Retrieves the data for row slice [i..j] from `row`.
  /// [`RowData`] **must** be created from unmodified `self` previously to
  /// guarentee safety.
  pub fn get_row(&self, row: &RowData) -> impl Iterator<Item = (&F, &usize)> {
    self.data[row.0[0]..row.0[1]].iter().zip_eq(&self.indices[row.0[0]..row.0[1]])
  }
  ...
  pub fn get_row_unchecked(&self, ptrs: &[usize; 2]) -> impl Iterator<Item = (&F, &usize)> {
    self.data[ptrs[0]..ptrs[1]].iter().zip_eq(&self.indices[ptrs[0]..ptrs[1]])
  }
  ...
  /// Multiply by a witness representing a dense vector; uses rayon to
  /// parallelize.
  ...
  /// Multiply by a witness representing a dense vector; uses rayon to
  /// parallelize. This does not check that the shape of the matrix/vector
  /// are compatible.
  #[tracing::instrument(skip_all, level = "trace", name = "SparseMatrix::multiply_vec_unchecked")]
  ...
  fn multiply_witness_into_unchecked(&self, W: &[F], u: &F, X: &[F], sink: &mut Vec<F>) {
    ...
    self
      ...
      .par_windows(2)
      .map(|ptrs| {
        self.get_row_unchecked(ptrs.try_into().unwrap()).fold(F::ZERO, |acc, (val, col_idx)| {
  ...
  pub fn len(&self) -> usize { *self.indptr.last().unwrap() }
  ...
  pub fn iter(&self) -> Iter<'_, F> {
    ...
    Iter { matrix: self, row, i: 0, nnz: *self.indptr.last().unwrap() }
  }

  pub fn num_rows(&self) -> usize { self.indptr.len() - 1 }
...
impl<F: PrimeField> Iterator for Iter<'_, F> {
  ...
  fn next(&mut self) -> Option<Self::Item> {
    ...
    let curr_item = (self.row, self.matrix.indices[self.i], self.matrix.data[self.i]);
