...
impl<E: Engine> NIFS<E> {
  ...
  /// an R1CS instance-witness tuple `(U2, W2)` with the same structure
  /// `shape` and defined with respect to the same `ck`, and outputs
  /// a folded Relaxed R1CS instance-witness tuple `(U, W)` of the same shape
  /// `shape`, with the guarantee that the folded witness `W` satisfies
  /// the folded instance `U` if and only if `W1` satisfies `U1` and `W2`
  /// satisfies `U2`.
  ///
  /// Note that this code is tailored for use with Nova's IVC scheme, which
  /// enforces certain requirements between the two instances that are
  /// folded. In particular, it requires that `U1` and `U2` are such that
  /// the hash of `U1` is stored in the public IO of `U2`.
  /// In this particular setting, this means that if `U2` is absorbed in the
  /// RO, it implicitly absorbs `U1` as well. So the code below avoids
  /// absorbing `U1` in the RO.
  ...
  pub fn prove(
    ...
    U2: &R1CSInstance<E>,
    W2: &R1CSWitness<E>,
  ) -> Result<(Self, (RelaxedR1CSInstance<E>, RelaxedR1CSWitness<E>), E::Scalar), NovaError> {
    ...
    // initialize a new RO
    let mut ro = E::RO::new(ro_consts.clone(), NUM_FE_WITHOUT_IO_FOR_NOVA_FOLD + io_arity);

    // append the digest of pp to the transcript
    ro.absorb(scalar_as_base::<E>(*pp_digest));

    // append U2 to transcript, U1 does not need to absorbed since U2.X[0] =
    // Hash(params, U1, i, z0, zi)
    ...
    Ok((Self { comm_T: comm_T.compress() }, (U, W), r))
  }

  /// Takes as input a Relaxed R1CS instance-witness tuple `(U1, W1)` and
  /// an R1CS instance-witness tuple `(U2, W2)` with the same structure
  /// `shape` and defined with respect to the same `ck`, and updates `(U1,
  /// W1)` by folding `(U2, W2)` into it with the guarantee that the
  /// updated witness `W` satisfies the updated instance `U` if and only
  /// if `W1` satisfies `U1` and `W2` satisfies `U2`.
  #[allow(clippy::too_many_arguments)]
  ...
  pub fn prove_mut(
    ...
    Ok((Self { comm_T: comm_T.compress() }, r))
  }
...
mod tests {
  ...
  fn test_tiny_r1cs_bellpepper_with<E: Engine>() {
    ...
    let ro_consts = <<E as Engine>::RO as ROTrait<
            <E as Engine>::Base,
            <E as Engine>::Scalar,
        >>::Constants::default();
    ...
    execute_sequence(&ck, &ro_consts, &<E as Engine>::Scalar::ZERO, &shape, &U1, &W1, &U2, &W2);
  }

  #[test]
  fn test_tiny_r1cs_bellpepper() { test_tiny_r1cs_bellpepper_with::<Bn256EngineKZG>(); }
  ...
  fn execute_sequence<E: Engine>(
    ...
    // produce a step SNARK with (W1, U1) as the first incoming witness-instance
    // pair
    ...
    // produce a step SNARK with (W2, U2) as the second incoming witness-instance
    // pair
  ...
  fn test_tiny_r1cs_with<E: Engine>() {
    ...
    // execute a sequence of folds
    execute_sequence(&ck, &ro_consts, &<E as Engine>::Scalar::ZERO, &S, &U1, &W1, &U2, &W2);
  }

  #[test]
  fn test_tiny_r1cs() { test_tiny_r1cs_with::<Bn256EngineKZG>(); }
}
