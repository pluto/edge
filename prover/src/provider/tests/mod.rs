...
pub mod solidity_compatibility_utils {
  ...
  where
    ...
    ec_points.iter().enumerate().for_each(|(i, ec_point)| {
      ...
      assert!(not_infinity);
      value.insert("i".to_string(), Value::String(i.to_string()));
      value.insert("x".to_string(), Value::String(format!("{:?}", coordinates_info.0)));
      value.insert("y".to_string(), Value::String(format!("{:?}", coordinates_info.1)));
      value_vector.push(Value::Object(value));
    });
    value_vector
  }
  ...
  where
    ...
    ec_points.iter().enumerate().for_each(|(i, ec_point)| {
      ...
      value.insert("i".to_string(), Value::String(i.to_string()));
      value.insert("compressed".to_string(), Value::String(format!("0x{}", hex::encode(data))));
      value_vector.push(Value::Object(value));
    });
    value_vector
  }
