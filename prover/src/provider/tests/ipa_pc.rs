...
mod test {
  ...
  use crate::provider::{
    ...
    pedersen::{CommitmentKey, CommitmentKeyExtTrait},
    tests::solidity_compatibility_utils::{
...
Grumpkin.GrumpkinAffinePoint memory commitment = Grumpkin.GrumpkinAffinePoint({{ commitment_x }}, \
                                                          {{ commitment_y }});

uint256 eval = {{ eval }};

return InnerProductArgument.IpaInputGrumpkin(ck_v, ck_s, point, L_vec, R_vec, commitment, eval, \
                                                          a_hat);
}
...
KeccakTranscriptLib.KeccakTranscript memory keccak_transcript = \
                                                          KeccakTranscriptLib.instantiate(label);
...
";
  ...
  // cargo test test_solidity_compatibility_ipa --release -- --ignored --nocapture
  // > ipa.t.sol
  #[test]
  #[ignore]
  fn test_solidity_compatibility_ipa() {
    ...
    map.insert("commitment_x".to_string(), Value::String(commitment_x_string));
    map.insert("commitment_y".to_string(), Value::String(commitment_y_string));
