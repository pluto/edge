...
/// This implementation behaves in ways specific to the halo2curves suite of
/// curves in:
// - to_coordinates,
...
macro_rules! impl_traits {
  ($name:ident, $order_str:literal, $base_str:literal) => {
    $crate::impl_traits!($name, $order_str, $base_str, cpu_best_msm);
  };
  ($name:ident, $order_str:literal, $base_str:literal, $large_msm_method:ident) => {
    // These compile-time assertions check important assumptions in the memory
    // representation of group data that supports the use of Abomonation.
    ...
    impl DlogGroup for $name::Point {
      ...
      // note: for halo2curves implementations, $name::Compressed == <$name::Point as
      // GroupEncoding>::Repr so the blanket impl<T> From<T> for T and impl<T>
      // Into<T> apply.
    ...
    impl<G: DlogGroup> TranscriptReprTrait<G> for $name::Compressed {
      fn to_transcript_bytes(&self) -> Vec<u8> { self.as_ref().to_vec() }
    }

    impl<G: Group> TranscriptReprTrait<G> for $name::Scalar {
      fn to_transcript_bytes(&self) -> Vec<u8> { self.to_repr().to_vec() }
    }
