//! This module implements Nova's evaluation engine using `HyperKZG`, a
//! KZG-based polynomial commitment for multilinear polynomials HyperKZG is based on the transformation from univariate PCS to multilinear PCS in the Gemini paper (section 2.4.2 in `<https://eprint.iacr.org/2022/420.pdf>`).
//! However, there are some key differences:
//! (1) HyperKZG works with multilinear polynomials represented in evaluation
//! form (rather than in coefficient form in Gemini's transformation).
//! This means that Spartan's polynomial IOP can use commit to its polynomials
//! as-is without incurring any interpolations or FFTs. (2) HyperKZG is
//! specialized to use KZG as the univariate commitment scheme, so it includes
//! several optimizations (both during the transformation of
//! multilinear-to-univariate claims and within the KZG commitment scheme
//! implementation itself). (3) HyperKZG also includes optimisation based on so called Shplonk/HaloInfinite technique (`<https://hackmd.io/@adrian-aztec/BJxoyeCqj#Phase-2-Gemini>`).
//! Compared to pure HyperKZG, this optimisation in theory improves prover (at
//! cost of using 1 fixed KZG opening) and verifier (at cost of eliminating MSM)
...
where
  ...
  // but are required since the equality constraints we use in the above do not transitively
  // carry bounds we should be able to remove most of those constraints when rust supports
  ...
  <E::G1 as Group>::Base: TranscriptReprTrait<E::G1>,
{
  ...
  fn get_batch_challenge(
    ...
    transcript.absorb(b"v", &v.iter().flatten().cloned().collect::<Vec<E::Fr>>().as_slice());
  ...
  E::G1Affine: TranscriptReprTrait<E::G1>,
{
  ...
  fn prove(
    ...
    // We do not compute final Pi (and its commitment as well since it is already
    // committed according to EvaluationEngineTrait API) as it is constant and
    // equals to 'eval' also known to verifier, so can be derived on its
    // side as well
    ...
    // Q(x), R(x) = P(x) / D(x), where D(x) = (x - r) * (x + r) * (x - r^2) = 1 *
    // x^3 - r^2 * x^2 - r^2 * x + r^4
    let D = UniPoly::new(vec![u[2] * u[2], -u[2], -u[2], E::Fr::from(1)]);
    let (Q_x, R_x) = batched_Pi.divide_with_q_and_r(&D).unwrap();

    let C_Q = <NE::CE as CommitmentEngineTrait<NE>>::commit(ck, &Q_x.coeffs).comm.to_affine();

    let a = Self::compute_a(&C_Q, transcript);

    // K(x) = P(x) - Q(x) * D(a) - R(a), note that R(a) should be subtracted from a
    ...
    let C_H = <NE::CE as CommitmentEngineTrait<NE>>::commit(ck, &h.coeffs).comm.to_affine();

    Ok(EvaluationArgument::<E> { comms, evals, R_x: R_x.coeffs, C_Q, C_H })
  }
  ...
  fn verify(
    ...
    // here we check that Pi polynomials were correctly constructed by the prover,
    // using 'r' as a random point, e.g: P_i_even = P_i(r) + P_i(-r) * 1/2
    // P_i_odd = P_i(r) - P_i(-r) * 1/2*r
    // P_i+1(r^2) == (1 - point_i) * P_i_even + point_i * P_i_odd -> should hold,
    // according to Gemini transformation
    ...
    let verification_failed = pi.evals[0]
      ...
      .any(|(index, ((eval_r, eval_minus_r), eval_r_squared))| {
        ...
        // P_i+1(r^2) == (1 - point_i) * (P_i(r) + P_i(-r)) * 1/2 + point_i * (P_i(r) -
        // P_i(-r)) * 1/2 * r is equivalent to:
        // 2 * r * P_i+1(r^2) == r * (1 - point_i) * (P_i(r) + P_i(-r)) + point_i *
        // (P_i(r) - P_i(-r))
    ...
    let D = UniPoly::new(vec![r_squared * r_squared, -r_squared, -r_squared, E::Fr::from(1)]);

    let a = Self::compute_a(&C_Q, transcript);

    let C_K = C_P - (C_Q * D.evaluate(&a) + vk.g * R_x.evaluate(&a));

    let pairing_inputs: Vec<(E::G1Affine, E::G2Prepared)> =
      vec![(C_H, vk.beta_h.into()), ((C_H * (-a) - C_K).to_affine(), vk.h.into())];

    #[allow(clippy::map_identity)]
    let pairing_input_refs = pairing_inputs.iter().map(|(a, b)| (a, b)).collect::<Vec<_>>();
...
mod tests {
  ...
  fn test_commitment_to_k_polynomial_correctness(
    ...
    let D = UniPoly::new(vec![r_squared * r_squared, -r_squared, -r_squared, Fr::from(1)]);
  ...
  fn test_d_polynomial_correctness(poly: &[Fr], point: &[Fr], _eval: &Fr) {
    ...
    let minus_R_x =
      UniPoly::new(R_x.clone().coeffs.into_iter().map(|coeff| -coeff).collect::<Vec<Fr>>());
    P_x += &minus_R_x;
  ...
  fn test_hyperkzg_shplonk_pcs_negative_wrong_commitment() {
    ...
    test_negative_inner_commitment(&poly, &point, &eval, &ck, &C2, &C2); // here we check detection
                                                                         // when proof was built
                                                                         // with wrong commitment
  }
  ...
  fn test_hyperkzg_shplonk_eval() {
    ...
    // The prover does not recompute so it may produce a proof, but it should not
    // verify
  ...
  fn test_hyperkzg_shplonk_more() {
    // test the hyperkzg prover and verifier with random instances (derived from a
    // seed)
  ...
  fn batch_challenge_powers(q: Fr, k: usize) -> Vec<Fr> { powers(&q, k) }
}
