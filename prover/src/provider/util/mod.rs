...
pub mod field {
  ...
  pub fn batch_invert<F: Field>(mut v: Vec<F>) -> Result<Vec<F>, NovaError> {
    ...
    let mut scratch_space = v
      .iter()
      .map(|x| if !x.is_zero_vartime() { Ok(*x) } else { Err(NovaError::InternalError) })
...
pub mod iterators {
  ...
  pub trait DoubleEndedIteratorExt: DoubleEndedIterator {
    /// This function employs Horner's scheme and core traits to create a
    /// combination of an iterator input with the powers
    ...
    where
      ...
      let Some(fst) = iter.next() else { panic!("input iterator should not be empty") };
...
pub mod test_utils {
  ...
  pub(crate) fn random_poly_with_eval<E: Engine, R: RngCore + CryptoRng>(
    ...
    let point = (0..num_vars).map(|_| <E as Engine>::Scalar::random(&mut rng)).collect::<Vec<_>>();
  ...
  /// Methods used to test the prove and verify flow of
  /// [`MultilinearPolynomial`] Commitment Schemes (PCS).
  ///
  /// Generates a random polynomial and point from a seed to test a
  /// proving/verifying flow of one of our [`EvaluationEngine`].
  ...
  fn prove_verify_with<E: Engine, EE: EvaluationEngineTrait<E>>(
    ...
    EE::verify(&verifier_key, &mut verifier_transcript, commitment, point, eval, &proof).unwrap();
    let pcv = verifier_transcript.squeeze(b"c").unwrap();

    // Check if the prover transcript and verifier transcript are kept in the same
    // state.
    assert_eq!(pcp, pcv);

    if evaluate_bad_proof {
      // Generate another point to verify proof. Also produce eval.
      let altered_verifier_point =
        point.iter().map(|s| s.add(<E as Engine>::Scalar::ONE)).collect::<Vec<_>>();
