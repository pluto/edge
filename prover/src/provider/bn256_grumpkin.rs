//! This module implements the Nova traits for `bn256::Point`, `bn256::Scalar`,
//! `grumpkin::Point`, `grumpkin::Scalar`.
use std::io::Read;
...
// Thus compile-time assertions checks important assumptions in the memory
...
mod tests {
  ...
  fn test_bn256_msm_correctness() {
    ...
    let scalars = (0..npoints).map(|_| bn256::Scalar::random(&mut rng)).collect::<Vec<_>>();
  ...
  fn test_grumpkin_msm_correctness() {
    ...
    let mut rng = thread_rng();
    let scalars = (0..npoints).map(|_| grumpkin::Scalar::random(&mut rng)).collect::<Vec<_>>();
