...
/// Trait for components with potentially discrete digests to be included in
/// their container's digest.
...
/// Marker trait to be implemented for types that implement `Digestible` and
/// `Serialize`. Their instances will be serialized to bytes then digested.
pub trait SimpleDigestible: Serialize {}

impl<T: SimpleDigestible> Digestible for T {
  fn write_bytes<W: Sized + io::Write>(&self, byte_sink: &mut W) -> Result<(), io::Error> {
    let config = bincode::DefaultOptions::new().with_little_endian().with_fixint_encoding();
...
impl<'a, F: PrimeField, T: Digestible> DigestComputer<'a, F, T> {
  fn hasher() -> Sha3_256 { Sha3_256::new() }
  ...
  pub fn new(inner: &'a T) -> Self { DigestComputer { inner, _phantom: PhantomData } }
...
mod tests {
  ...
  use crate::{provider::GrumpkinEngine, traits::Engine};
  ...
  impl<E: Engine> S<E> {
    fn new(i: usize) -> Self { Self { i, digest: OnceCell::new() } }

    fn digest(&self) -> E::Scalar {
      self.digest.get_or_try_init(|| DigestComputer::new(self).digest()).cloned().unwrap()
    }
  }
  ...
  fn test_digest_field_not_ingested_in_computation() {
    ...
    assert_eq!(
      DigestComputer::<<E as Engine>::Scalar, _>::new(&s1).digest().unwrap(),
      DigestComputer::<<E as Engine>::Scalar, _>::new(&s2).digest().unwrap()
    );

    // note: because of the semantics of `OnceCell::get_or_try_init`, the above
    // equality will not result in `s1.digest() == s2.digest`
    assert_ne!(s2.digest(), DigestComputer::<<E as Engine>::Scalar, _>::new(&s2).digest().unwrap());
  }
