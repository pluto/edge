  ...
  S2: RelaxedR1CSSNARKTrait<Dual<E1>>,
{
  ...
  pub fn setup(
    ...
    let prover_key = ProverKey { pk_primary, pk_secondary };
    let verifier_key = VerifierKey { vk_primary, vk_secondary };
  ...
  pub fn prove(
    ...
    // Prepare the list of primary Relaxed R1CS instances (a default instance is
    // provided for uninitialized circuits)
    ...
    // Prepare the list of primary relaxed R1CS witnesses (a default witness is
    // provided for uninitialized circuits)
    let r_W_primary: Vec<RelaxedR1CSWitness<E1>> = recursive_snark
      ...
      .enumerate()
      .map(|(idx, r_W)| {
        r_W.clone().unwrap_or_else(|| RelaxedR1CSWitness::default(&pp[idx].r1cs_shape))
      })
      .collect::<Vec<_>>();
  ...
  pub fn verify(
    ...
    // secondary circuit
    // NOTE: This count ensure the number of witnesses sent by the prover must equal
    // the number of NIVC circuits
    ...
    // Compute the primary and secondary hashes given the digest, program counter,
    // instances, and witnesses provided by the prover
    ...
    // Compare the computed hashes with the public IO of the last invocation of
    // `prove_step`
    ...
    let res_secondary = self.f_W_snark_secondary.verify(&vk.vk_secondary, &f_U_secondary);
...
mod test {
  ...
  impl<E: Engine> StepCircuit<E::Scalar> for SquareCircuit<E> {
    ...
    ) -> Result<(Option<AllocatedNum<E::Scalar>>, Vec<AllocatedNum<E::Scalar>>), SynthesisError>
    {
      let z_i = &z[0];
  ...
  impl<E: Engine> StepCircuit<E::Scalar> for CubeCircuit<E> {
    ...
    ) -> Result<(Option<AllocatedNum<E::Scalar>>, Vec<AllocatedNum<E::Scalar>>), SynthesisError>
    {
      ...
      cs.enforce(|| "next_pc = 0", |lc| lc + CS::one(), |lc| lc + next_pc.get_variable(), |lc| lc);
  ...
  where
    ...
    for circuit in test_circuits.iter().take(num_steps) {
      recursive_snark.prove_step(&pp, circuit, &secondary_circuit).unwrap();
    ...
    compressed_snark.verify(&pp, &verifier_key, &z0_primary, &z0_secondary).unwrap();
  }
