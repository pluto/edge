//! Supernova implementation support arbitrary argumented circuits and running
//! instances. There are two Verification Circuits for each argumented circuit:
//! The primary and the secondary. Each of them is over a cycle curve but
//! only the primary executes the next step of the computation.
//! Each circuit takes as input 2 hashes.
//! Each circuit folds the last invocation of the other into the respective
//! running instance, specified by `augmented_circuit_index`
//!
...
//!    3. (optional by F logic) F circuit might check `program_counter_{i}` invoked current F
//!       circuit is legal or not.
//!    3. F circuit produce `program_counter_{i+1}` and sent to next round to optionally constraint
//!       the next F' argumented circuit.
...
pub trait StepCircuit<F: PrimeField>: Send + Sync + Clone {
  ...
  /// Return this `StepCircuit`'s assigned index, for use when enforcing the
  /// program counter.
...
/// used internally to enforce that the circuit selected by the program counter
...
impl SuperNovaAugmentedCircuitParams {
  ...
  pub fn get_n_limbs(&self) -> usize { self.n_limbs }
}
...
impl<'a, E: Engine> SuperNovaAugmentedCircuitInputs<'a, E> {
  ...
  pub fn new(
    ...
    Self { pp_digest, i, z0, zi, U, u, T, program_counter, last_augmented_circuit_index }
  }
}

/// The augmented circuit F' in `SuperNova` that includes a step circuit F
/// and the circuit for the verifier in `SuperNova`'s non-interactive folding
/// scheme, `SuperNova` NIFS will fold strictly r1cs instance u with respective
/// relaxed r1cs instance `U[last_augmented_circuit_index]`
...
impl<'a, E: Engine, SC: EnforcingStepCircuit<E::Base>> SuperNovaAugmentedCircuit<'a, E, SC> {
  ...
  pub const fn new(
    ...
    Self { params, inputs, step_circuit, ro_consts, num_augmented_circuits }
  }
  ...
  > {
    ...
    let program_counter = if self.params.is_primary_circuit {
      Some(AllocatedNum::alloc(cs.namespace(|| "program_counter"), || {
        Ok(self.inputs.get()?.program_counter.expect("program_counter missing"))
      })?)
    } else {
      None
    };
    ...
    let U = (0..num_augmented_circuits)
      .map(|i| {
        AllocatedRelaxedR1CSInstance::alloc(
          ...
          self.inputs.as_ref().and_then(|inputs| inputs.U.and_then(|U| U[i].as_ref())),
    ...
    Ok((params, i, z_0, z_i, U, u, T, program_counter, last_augmented_circuit_selector))
  }
  ...
  ) -> Result<(Vec<AllocatedRelaxedR1CSInstance<E, NIO_NOVA_FOLD>>, AllocatedBit), SynthesisError>
  {
    ...
    let U_next: Vec<AllocatedRelaxedR1CSInstance<E, NIO_NOVA_FOLD>> =
      zip_with!((U.iter(), last_augmented_circuit_selector.iter()), |U, equal_bit| {
        conditionally_select_alloc_relaxed_r1cs(
  ...
  pub fn synthesize<CS: ConstraintSystem<<E as Engine>::Base>>(
    ...
    let num_augmented_circuits = if self.params.is_primary_circuit {
      // primary circuit only fold single running instance with secondary output
      // strict r1cs instance
      1
    } else {
      // secondary circuit contains the logic to choose one of multiple augments
      // running instance to fold
      self.num_augmented_circuits
    };
    ...
    if self.inputs.is_some() {
      ...
      let last_augmented_circuit_index =
        self.inputs.get().map_or(E::Base::ZERO, |inputs| inputs.last_augmented_circuit_index);
    ...
    // must use return `last_augmented_circuit_index_checked` since it got range
    // checked
    ...
    // The next RunningInstance folding can take the pre-image of this hash as
    // witness and check.

    // "Finally, there is a subtle sizing issue in the above description: in each
    // step, because Ui+1 is produced as the public IO of F0
    // program_counter+1, it must be contained in the public IO of instance
    // ui+1. In the next iteration, because ui+1 is folded
    // into Ui+1[program_counter+1], this means that Ui+1[program_counter+1] is at
    // least as large as Ui by the properties of the folding scheme. This
    // means that the list of running instances grows in each step. To
    // alleviate this issue, we have each F0j only produce a hash
    ...
    if program_counter.is_some() {
      ro.absorb(program_counter_new.as_ref().expect("new program counter missing"))
    }
    for e in &z_0 {
      ro.absorb(e);
    }
    ...
    // We are cycling of curve implementation, so primary/secondary will rotate hash
    // in IO for the others to check bypass unmodified hash of other circuit
    // as next X[0] and output the computed the computed hash as next X[1]
