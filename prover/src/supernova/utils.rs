...
///
/// This function assumes `selector_vec` has been properly constrained", i.e.
/// that exactly one entry is equal to 1.
// NOTE: When `a` is greater than 5 (estimated), it will be cheaper to use a
// multicase gadget.
//
// We should plan to rely on a well-designed gadget offering a common interface
// but that adapts its implementation based on the size of inputs (known at
// synthesis time). The threshold size depends on the size of the elements of
// `a`. The larger the elements, the fewer are needed before multicase becomes
// cost-effective.
pub fn get_from_vec_alloc_relaxed_r1cs<E: Engine, CS: ConstraintSystem<<E as Engine>::Base>>(
  ...
  let selected = a.iter().zip_eq(selector_vec.iter()).enumerate().skip(1).try_fold(
    first,
    |matched, (i, (candidate, equal_bit))| {
